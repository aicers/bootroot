...
assert_functionality_and_audit() {
  ...
  REGISTRATION_ID="$(jq -r '.registration_id' <<<"$first")"; role="$(sudo -n curl -fsS --cacert "$ROOT_CA" -K "$TOKEN_CURL" "$OPENBAO_URL/v1/auth/approle/role/$REGISTRATION_ID")"; policies="$(jq -c '.data.token_policies | sort' <<<"$role")"; [ "$policies" = "[\"bootroot-service-${REGISTRATION_ID}\"]" ] || fail "minted role policy set is not exactly the derived service policy"
  ...
  sudo -n curl -fsS --cacert "$ROOT_CA" -K "$TOKEN_CURL" "$OPENBAO_URL/v1/auth/approle/role/redteam-review" >"$ARTIFACT_DIR/refused-role.json" 2>&1 && fail "refused mint created an AppRole"
  sudo -n curl -fsS --cacert "$ROOT_CA" -K "$TOKEN_CURL" "$OPENBAO_URL/v1/sys/policies/acl/bootroot-service-redteam-review" >"$ARTIFACT_DIR/refused-policy.json" 2>&1 && fail "refused mint created a policy"
  sudo -n curl -fsS --cacert "$ROOT_CA" -K "$TOKEN_CURL" "$OPENBAO_URL/v1/secret/data/bootroot/services/redteam-review/secret_id" >"$ARTIFACT_DIR/refused-kv.json" 2>&1 && fail "refused mint wrote service KV"
