...
- `remote-bootstrap`: 서비스가 다른 머신에 있을 때 사용하며, 서비스 머신의
  ...
  반영합니다. OpenBao Agent 사이드카는 설치하지 않으며, 원격 `bootroot-agent`가
  AppRole로 직접 인증합니다. 초기 bootstrap 이후에는 실행 중인 에이전트가
  스스로를 유지합니다. fast-poll 루프가 자신의 `secret_id`를 갱신하고 trust를
  OpenBao에서 다시 렌더링하므로, `secret_id` 회전에 원격 호스트의 운영 조치가
  필요하지 않습니다. `bootroot-remote apply-secret-id`는 `secret_id_ttl`이
  지나도록 오프라인이어서 자격 증명이 이미 만료된 에이전트를 복구하는
  경로일 뿐입니다.

두 delivery mode 모두 의도하는 모델은 같습니다. 먼저 trust를 준비한 뒤,
검증이 켜진 상태로 `bootroot-agent`를 시작하는 것입니다. `local-file`은
같은 step-ca 호스트에서 이를 직접 반영하고, `remote-bootstrap`은 초기 trust를
`bootroot-remote bootstrap`으로 서비스 호스트에 반영한 뒤 실행 중인 에이전트가
이후 fast-poll 루프로 trust를 직접 다시 렌더링합니다.
