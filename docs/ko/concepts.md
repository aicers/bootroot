...
런타임에 시크릿을 최신으로 유지하는 흐름은 두 갈래로 나뉩니다.

**인프라 구성 요소(step-ca, HTTP-01 responder).** 이들은 OpenBao에 직접
접속하지 않습니다. `bootroot init`이 구성 요소별 전용 **OpenBao Agent**
(`openbao-agent-stepca`, `openbao-agent-responder`)를 준비합니다.

- OpenBao Agent가 AppRole(role_id + secret_id)로 로그인
- 필요한 시크릿을 템플릿 파일로 렌더링(예: `password.txt`,
  `responder.toml`)
- step-ca와 HTTP-01 responder가 렌더된 파일을 읽어 사용

**런타임 서비스(bootroot-agent).** 서비스별 OpenBao Agent는 실행되지
않습니다. `bootroot-agent`가 AppRole로 OpenBao에 직접 인증하고,
**fast-poll 루프**가 KV에서 다음 값들을 스스로 최신으로 유지합니다.

- trust 앵커(`ca-bundle.pem`과 `agent.toml`의 `trusted_ca_sha256` 핀)
- 자신의 `secret_id`(재로그인 때마다 디스크에서 다시 읽고, 루프가 KV에서
  갱신)
- HTTP-01 responder HMAC
- EAB 자격증명(`eab.json`에 기록/삭제하며 `--eab-file`로 소비)

`bootroot service add --delivery-mode local-file`은 이 루프를 활성화하는
`[openbao]` 섹션(url, kv_mount, role_id_path, secret_id_path,
ca_bundle_path, state_path)을 서비스의 `agent.toml`에 기록합니다.
`bootroot-remote bootstrap`이 원격 호스트에 준비하는 것과 같은 메커니즘이라,
두 delivery mode가 하나의 시크릿 전달 경로를 공유합니다.
...
초기 설정 이후에는 인프라 OpenBao Agent와 각 서비스의 `bootroot-agent`가
**AppRole**(role_id + secret_id)로
...
위해서는 서비스별로 bootroot-agent가 올바르게 설정되어야
하며, 이 에이전트 설정을 `bootroot service add`가 어떤 경로로 반영할지는
서비스가 step-ca 설치 머신에 함께 있는지 또는 다른 머신에 있는지에 따라
달라집니다.

- 같은 머신: `bootroot` CLI가 bootroot-agent 관련 로컬 설정 파일
  반영을 직접 처리할 수 있습니다.
...
- `local-file`: 서비스가 step-ca 설치 머신에 함께 있을 때 사용합니다.
  CLI가 `[openbao]` fast-poll 섹션을 포함한 에이전트 설정을 직접 기록하고,
  서비스의 `secret_id` 옆에 `eab.json`을 준비합니다.
...
시작하는 것입니다. 어느 모드에서도 서비스별 OpenBao Agent는 설치되지
않으며, 실행 중인 에이전트는 스스로를 유지합니다. fast-poll 루프가 자신의
`secret_id`, responder HMAC, EAB를 갱신하고 trust를 OpenBao에서 다시
렌더링하므로, 회전에 서비스 호스트의 운영 조치가 필요하지 않습니다.
`local-file`은 초기 trust를 step-ca 호스트에서 직접 반영하고,
`remote-bootstrap`은 `bootroot-remote bootstrap`으로 서비스 호스트에
반영합니다.
