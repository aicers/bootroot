...
거부되며, 그 파일에 마지막으로 기록된 실행을 알려 줍니다. 거부된 실행이나
잠금을 잡기 전에 실패한 실행은 끝날 때 `/etc/hosts`를 건드리지 않습니다. 정리
시의 고쳐 쓰기는 그 스크립트의 마커가 붙은 줄을 모두 지우므로, 어느 실행의
entry인지 구별할 수 없기 때문입니다.

그 잠금을 쥐는 것은 파일의 존재가 아니라 열린 파일 서술자에 걸린 권고 잠금,
즉 `flock(2)`입니다. 소유권은 커널의 것이고 실행이 사는 동안 그대로 유지되므로
여기에는 오래된 잠금이라는 것이 없습니다. `SIGKILL`로 종료된 실행은
`/etc/hosts`를 쓸 수 없게 되는 바로 그 순간에 잠금을 놓고, 다음 실행은 회수할
것도 판단할 것도 없이 그것을 가져갑니다. 이것이 pid 파일로는 할 수 없는
일입니다. pid 파일을 회수한다는 것은 조금 전에 읽은 pid를 근거로 파일을
...
잠금이 파일이 아니라 서술자라는 사실에서 두 가지가 따라옵니다. 그 경로의
파일은 한 번 만들어진 뒤 지워지지 않습니다. 삭제를 사이에 두고 그 경로를 연 두
실행은 서로 다른 inode를 잠그게 되어 어느 쪽도 충돌하지 않기 때문입니다. 그
파일이 담는 것은 라벨(pid와, 그것을 쓴 실행의 하네스 및 아티팩트 디렉터리)
뿐이며, 거부 메시지가 그것을 알려 줄 뿐 무엇도 그 값에 따라 움직이지 않습니다.
그리고 실행이 시작한 모든 자식이 그 서술자를 물려받으므로, 하네스는 자신이
띄우는 `bootroot-agent` 데몬에 대해 그것을 닫습니다(`9>&-`). 종료된 실행보다
오래 사는 데몬이 있으면 그 잠금을 계속 쥐고 있게 되기 때문입니다.
...
없으므로, 누군가 그 경로에 파일을 심어 두면 철거를 겨냥하는 대신 `hosts` 실행을
요란하게 거부하게 됩니다. 마커 디렉터리에 있는 소유권 검사 장치가 여기에는
필요 없는 이유입니다.

사용자 사이에서도 더 필요한 것은 없습니다. 그 파일은 만든 사람의 것이고 그
실행보다 오래 남으므로, 다음 실행은 거기에 쓸 수도 모드를 바꿀 수도 없습니다.
그러나 `flock(2)`는 어떻게 열었든 서술자를 잠그므로 읽기 전용으로 여는 것으로
충분하고, 자기 것이 아닌 잠금 파일을 쥔 실행은 라벨을 그대로 둡니다. 잠금은
만들 때의 umask와 무관하게 누구나 읽을 수 있게 기록합니다. 잠금을 잡는다는
것이 곧 그 파일을 여는 일이므로, 0600으로 남은 잠금은 그 호스트의 다른 모든
사용자에게 `hosts` 모드를 영영 거부하기 때문입니다. 여기서는 sticky `/tmp`에
있는 남의 파일을 지우거나 이름을 바꾸는 일이 없으므로, 이 잠금은 `/etc/hosts`를
고치기 위해 이 모드가 이미 요구하는 `sudo -n` 외에 어떤 권한도 따로 요구하지
않습니다.
