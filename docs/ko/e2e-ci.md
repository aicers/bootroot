...
리스너 전환, 기록된 `https://` URL, 여섯 개 파일로 이루어진 자격 증명 집합(보호
대상 다섯 개는 uid 0 / gid 0 / `0600`, 그 옆의 전용 CA 번들), 내부 SAN이 해석되는
리스폰더 별칭, 그리고 `init`이 방금 발급한 자격 증명으로 수행하는 실제
`auth/cert/login`을 확인하며, 클라이언트 인증서 없이 같은 로그인을 시도하면
거부되는지도 함께 확인합니다. 소유권 분리의 반대쪽도 확인합니다. `OpenBao` Agent
사이드카의 설정, `AppRole` 쌍, 템플릿, 그리고 이들을 담은 디렉터리는 여전히
`secrets/`의 소유자에게 속해야 하며, root로 실행한 `init`이 이를 가져가서는
안 됩니다.
그 사이드카가 무엇을 향해 생성되었는지도 확인합니다. 이 시나리오는 `OpenBao`를
TLS로 전환하며, 두 에이전트는 전환 전에 파일이 작성되고 전환 후에 기동되므로
`agent.hcl`과 compose 오버라이드는 `https://` 주소와 리스너 리프 인증서를
검증할 CA 번들을 가리켜야 합니다. 평문 리스너를 향해 생성해도 기동 시점에는
드러나지 않습니다. 컨테이너는 뜨고 `init`도 성공하지만, 이후 에이전트는 TLS
포트에 평문으로 말하며 아무것도 갱신하지 못합니다.

여기서 `init`은 `sudo -n env`를 통해 root로 실행됩니다. 엔드포인트가 활성화된
`init`은 그 다섯 개 파일을 `root:root`로 게시하며, 그러지 못하면 어느 것도
게시하지 않기 때문입니다. **따라서 비밀번호 없는 sudo가 이 시나리오의
전제 조건이며**, 아무것도 설치하기 전에 전제 조건 블록에서 확인합니다. 그 이후
root 소유 `0700` 내부 디렉터리를 읽는 작업은 실행 전체를 승격하지 않고 필요한
호출마다 `sudo -n`을 사용합니다.

그런 다음 방금 프로비저닝한 배포 위에서 `bootroot infra up`을 실행하고 두 가지
소유권 검사를 한 번 더 수행합니다. `infra up`은 재귀적 소유권 정리로 끝납니다.
`secrets/` 아래 모든 항목을 그 디렉터리 자신의 소유자로 chown하는 일회성 root
컨테이너인데, 그래서 아무것도 다시 게시하지 않고도 root 소유권을 되돌릴 수 있는
일상적인 명령이기도 합니다. bootroot 내부 디렉터리는 이 정리 대상에서 제외되며,
그 사실이 지켜지는지 확인하는 곳이 바로 여기입니다. 보호 대상 다섯 개는 그
...
audit_store_dir`이 저장소를 지목하고 `[registrar_endpoint] enabled = true`로
두 활성화 출처를 일치시킵니다. 두 값이 다르면 `init`이 진행을 거부하기
때문입니다. 이 시나리오는 `bootroot-agent`를 기동하지 않으므로, 설치 측만 읽는
파일에서 `[registrar_endpoint]`가 활성이어도 아무것도 시작되지 않습니다. `init`
이후에는 저장소가 root 소유 `0700`이므로 `sudo -n`을 통해 다음을 확인합니다.
저장소가 `records/`, `openbao/`와 함께 존재하는지, 저장소와 `records/`는
`0:0:700`인데 `openbao/`는 컨테이너 사용자에게 넘어갔는지, 렌더링된 오버라이드가
`<audit_store_dir>/openbao`를 바인딩하고 OpenBao의 나머지 두 마운트를 다시
선언하는지, 그리고 실행 중인 컨테이너의 `/openbao/audit`이 호스트 저장소의 bind
mount인지입니다. 마지막의 권한 없는 `infra up`은 이 확인들을 반복하고 — 내부로
들어갈 수 없는 저장소에 대해 선택 경로가 동작한다는 증거입니다 — 이어서 다른
라이프사이클 하네스가 모두 실행하는 공용 파일 감사 확인
`assert_openbao_audit_log`를 그대로 실행합니다. 이 헬퍼는 `docker exec`로
*컨테이너* 경로를 읽으므로, 올바른 bind mount는 헬퍼에게 투명합니다.

저장소의 **예약량(reserve)**을 검증하는 것도 이 시나리오뿐이며, 세 부분으로
나뉩니다. 첫 번째는 호스트에 아무것도 요구하지 않습니다. 자기 전용 저장소
경로에 대해 엔드포인트가 켜진 `filesystem` 모드 `bootroot init`은
`provisioned, not activated`를 보고하고 멈춰야 하며, 이때 마운트 지점만
만들고 두 하위 디렉터리도, 이미지도, `/etc/systemd/system` 아래의 어떤 파일도
만들지 않아야 합니다 — 세 산출물은 Compose 오버라이드 옆에 기록될 뿐 아무도
설치하지 않습니다. 두 번째는 실제로 활성화하며, 호스트가 이를 감당할 수 있을
때만 실행됩니다. 생성된 `.mount` 유닛을 enable할 살아 있는 systemd,
`mkfs.ext4`, `dd`, `losetup`, 그리고 loop 장치가 필요합니다. `fallocate`는
선택 사항이며, 사용할 수 없으면 렌더링된 할당 명령이 `dd`로 대체합니다. 여기서는
렌더링된 phase 2 명령을 결과 출력에서 그대로 읽어 출력된 문자열 그대로
셸에 붙여 실행합니다 — 운영자가 그렇게 할 수 있다는 증거가 바로 이것입니다 —
그리고 이어지는 재실행이 `enforced`를 보고해야 합니다. 유닛 이름이
`systemd-escape --path`와 바이트 단위로 같은지, 로드된 유닛의 `Where=`가 설정된
저장소인지, 마운트가 파생된 이미지를 backing file로 갖는 loop 장치인지,
마운트가 올라온 직후와 재실행 뒤에 각각 이미지가 완전히 할당되어 있는지 —
앞쪽은 `-E nodiscard`가 빠진 구현이, 뒤쪽은 `-E lazy_itable_init=0`이 빠진
구현이 실패하는 확인입니다. 커널의 백그라운드 아이노드 테이블 처리는 마운트
몇 초 뒤에야 그 블록을 돌려주기 때문입니다 —, 마운트된 파일시스템 위에서
저장소 디렉터리와 두
하위 디렉터리가 각자의 계약이 정한 소유자와 모드를 갖는지 — 특히 저장소
디렉터리는 `mkfs.ext4`가 파일시스템 루트를 `0755`로 만들고 렌더링된 4단계가
그것을 `0700`으로 되돌리는 곳이므로 —, 두 번째 실행이 이미지 명령을 하나도
렌더링하지 않고 이미지를 이미 갖고 있던 아이노드와 크기, 파일시스템 UUID 그대로 남겨두는지, 그리고 예약량의 네 배를 쓰는 시도가 예약량
쪽에서 실패하고 호스트 루트 파일시스템이 그것을 흡수하지 않는지를 확인합니다.
정리 단계는 유닛을 disable하고 언마운트한 뒤 유닛과 두 drop-in, 이미지를
제거하며, 섹션 끝뿐 아니라 cleanup 트랩에서도 실행되므로 중간에 실패해도
저장소 상위 디렉터리 삭제가 마운트 안으로 들어가는 일이 없습니다. systemd가
없는 호스트 — macOS 프리플라이트나 컨테이너 실행기 — 는 건너뛴다는 줄을 남기고
지나갑니다.

세 번째는 활성화된 예약량 위에 *배포*를 올리며, 두 번째와 같은 호스트 조건에서만
실행됩니다. 실행 순서상 맨 마지막인데, 이 배포의 감사 장치를 위의 모든 확인이
대상으로 삼는 `directory` 모드 저장소에서 옮겨 오고, 마지막에는 실행 중인
컨테이너 아래에서 예약량을 가득 채우기 때문입니다. 먼저 이전 저장소를 가리키는
렌더링된 오버라이드를 제거하고 — `bootroot init`은 `--agent-config`가 다른
저장소를 해석하는 실행을 거부합니다 — 두 번째와 똑같은 방식으로 자기 예약량을
활성화한 뒤, 1단계가 준비된 산출물을 교체할 수 있도록 root로
`bootroot infra up`을 실행해 그 위에 스택을 올립니다. 이 bring-up 자체가 하나의
확인입니다. 오버라이드에서 bind 원본을 읽어 그 저장소 디렉터리를 검사하는데,
마운트가 올라온 상태에서 그것은 *마운트된 파일시스템의 루트*이므로, 4단계에서
`audit_store_dir`에 `chmod 0700`을 다시 적용하지 않은 실행은 바로 다음에 오는 이
명령에서 거부됩니다. 이어서 컨테이너의 `/openbao/audit`이 마운트된 저장소의 bind
mount인지, 두 하위 디렉터리와 감사 로그 자체가 마운트된 파일시스템 위에 있는지,
그리고 공용 확인 `assert_openbao_audit_log`가 — 그대로, 바뀐 것 없이 — 그 장치에
대해 통과하는지를 확인합니다. 자동 부팅 경로가 기대는 순서 관계는 설치된 파일이
아니라 systemd가 실제로 로드한 유닛에서 다시 읽습니다. `docker.service`가 마운트 유닛을
`Wants=`하고 그 뒤에 순서를 두며 강한 관계는 갖지 않는다는 것으로, 두 매뉴얼이
남긴 잔여 위험 그대로이지 그보다 강한 보장이 아닙니다. 그다음 스택을 멈췄다가
마운트 위로 다시 올립니다. 마운트 유닛은 여전히 active이고, 재시작된 컨테이너는
여전히 예약량에 바인딩되어 있으며, 감사 로그는 *늘어납니다*. 내용이 아니라
증가량인 이유는 이 장치가 이어 쓰기 때문입니다 — 이전 컨테이너가 남긴 항목만으로
공용 확인은 통과하며, 새 컨테이너가 쓰고 있음을 보여 주는 것은 증가량뿐입니다.
그 뒤의 `bootroot init`은, 그 시점에는 Compose 엔트리포인트가 `openbao/`를
컨테이너 사용자로 chown한 뒤인데도 여전히 `enforced`를 보고해야 합니다. 소유자를
비교하는 구현이 실패하는 확인입니다. 마지막은 다시 상한선이며, 이번에는 배포를
상대로 합니다. 예약량의 네 배를 쓰는 시도가 예약량 쪽에서 실패하고 호스트 루트
파일시스템은 그것을 흡수하지 않습니다. 활성화는 cleanup 트랩까지 호스트에
남습니다. 컨테이너가 마운트된 저장소 안으로 바인딩되어 있어 실행 중에는 언마운트가
거부되기 때문입니다.

세 부분 모두 `bootroot init`을 아무도 응답하지 않는 URL로 실행하며, 앞의 두
부분은 끝난 뒤 Compose 오버라이드를 지웁니다. 어느 쪽도 우연이 아닙니다. 감사
저장소는 Docker
호출보다 먼저 프로비저닝되므로 `enforced`에 도달한 실행은 그대로 초기화 본
과정으로 이어져, 진짜 `init`보다 먼저 이 시나리오의 OpenBao를 초기화해
버립니다 — 응답 없는 URL은 결과가 출력된 뒤, 배포의 어떤 것도 건드리기 전에
OpenBao 헬스 체크에서 실행을 멈춥니다. 그리고 오버라이드는 결과를 검증하기
*전에* 렌더링되므로, `provisioned, not activated`에서 멈춘 실행조차 자기
임시 저장소를 가리키는 오버라이드를 남깁니다. `bootroot init`은 렌더링된
오버라이드를 다음 `--agent-config`가 해석한 저장소와 대조해 둘이 다르면
거부하므로, 남겨 두면 이후의 모든 실행이 자기 판정 대신 그 오류로 거부됩니다.
세 번째 부분은 같은 규칙의 반대쪽입니다. 자기 저장소를 프로비저닝하기 전에 배포가
초기화될 때 쓰인 오버라이드를 제거하고, 자기가 렌더링한 오버라이드는 그대로
남깁니다. 바로 다음 줄의 bring-up이 그 파일에서 bind 원본을 읽기 때문입니다.

저장소의 상위 디렉터리는 run root가 아니라 물리적이고 world-traversable한 임시
디렉터리입니다. `bootroot init`은 저장소 위의 모든 기존 경로 구성요소가 심볼릭
링크가 아닌 디렉터리이고 `o+x`를 가질 것을 요구하는데, 떠오르는 위치는 모두 그
규칙에 걸리기 때문입니다. `RUN_ROOT`는 `mktemp -d`라 `0700`이고, macOS에서
`$TMPDIR`은 사용자별 `0700` 디렉터리이며 논리적 `/tmp`는 심볼릭 링크이고,
체크아웃 아래는 흔히 `0750`인 홈 디렉터리를 물려받습니다. 그래서 하네스는
`/tmp`를 물리적으로 해석하고, `$TMPDIR`이 전혀 사용되지 않도록 명시적인
`mktemp -d` 템플릿으로 그 아래에 기준 디렉터리를 만든 뒤 `chmod 0755`하고,
`init` 실행 전에 그 경로의 모든 구성요소를 스스로 검사합니다. 임시 디렉터리
구성이 달라지면 `init`의 상위 디렉터리 오류가 아니라 하네스 자신의 메시지로
실패하게 하기 위해서입니다. 정리 단계는 이 시나리오가 이미 요구하는 같은
`sudo -n`으로 그 기준 디렉터리를 제거하고, 남아 있으면 leftover로 보고합니다.
...
실패합니다. 그 시나리오들은 저장소도 프로비저닝하지 않습니다. 기록된 술어가
없으니 오버라이드도 렌더링되지 않고, OpenBao 감사 로그는 `openbao-audit` 명명
볼륨에 그대로 남습니다.
...
마친 뒤 첫 수정 전에 `/etc/hosts` 자체에 배타적 `flock(2)`을 걸고, 자신의
entry를 모두 지운 뒤에 놓습니다. 두 번째 실행은 파일에 손대기 전에 그 지점에서
거부되며, 알아낼 수 있는 경우에는 어느 실행이 쥐고 있는지도 알려 줍니다. 거부된
실행이나 잠금을 잡기 전에 실패한 실행은 끝날 때 `/etc/hosts`를 건드리지
않습니다. 정리 시의 고쳐 쓰기는 그 스크립트의 마커가 붙은 줄을 모두 지우므로,
어느 실행의 entry인지 구별할 수 없기 때문입니다.

잠금의 대상이 곧 보호 대상이라는 것이 이 설계의 전부입니다. 두 가지 성질이
그것을 떠받치며, 각각이 다른 쪽으로는 막지 못하는 실패를 막습니다.

소유권은 커널의 것이고 실행이 사는 동안 그대로 유지되므로 오래된 잠금이라는
것이 없습니다. `SIGKILL`로 종료된 실행은 `/etc/hosts`를 쓸 수 없게 되는 바로
그 순간에 잠금을 놓고, 다음 실행은 회수할 것도 판단할 것도 없이 그것을
가져갑니다. 이것이 "이 경로가 존재한다"를 소유권으로 삼는 잠금 파일로는 할 수
없는 일입니다. 그런 잠금을 회수한다는 것은 조금 전에 읽은 pid를 근거로 파일을
지운다는 뜻인데, 그 사이에 같은 오래된 잠금에 도달한 두 번째 실행이 그것을
가져갈 수 있고, 그러면 회수는 살아 있는 실행의 잠금을 없애 두 실행이 모두
자기가 뮤텍스를 쥐었다고 믿으며 `/etc/hosts`를 고치게 됩니다.

그리고 그 inode는 누구도 바꿔치기할 수 없습니다. `flock(2)`이 잠그는 것은
inode이고, 경로가 어느 inode를 가리키는지는 아무도 그것을 바꾸지 않는 동안에만
유지됩니다. 그래서 누구나 쓸 수 있는 `/tmp`의 예측 가능한 이름에 하네스가 만든
잠금 파일은 쪼개질 수 있는 뮤텍스입니다. 심볼릭 링크를 거부하는 검사와 그
뒤의 `open`은 두 번의 호출이고, 머신의 누구든 그 사이에 링크를 심을 수
있습니다. 한 실행은 링크가 가리키는 것을 잠그고, 링크가 다시 평범한 파일로
바뀐 뒤 도착한 다음 실행은 다른 inode를 잠그면서 첫 실행이 쥐고 있다고 믿는
뮤텍스를 가져갑니다. `O_NOFOLLOW`는 이 틈을 좁힐 뿐 닫지 못합니다. sticky
`/tmp`에서 남이 소유한 파일은 두 실행의 `open` 사이에 그 소유자가 지우고 다시
만들 수 있기 때문입니다. `/etc/hosts`는 root의 디렉터리에 있으므로 권한 없는
사용자가 그 경로에 다른 inode를 놓을 수 없고, 그것을 잠그는 실행은 곧 그것을
수정할 실행입니다.

이 잠금은 아무것도 만들지 않고, 쓰지 않으며, 모드를 바꾸지 않습니다. 여는
방식은 읽기 전용인데, 이는 `flock(2)`에 필요한 전부이자 `sudo` 없이 어떤
실행이든 `/etc/hosts`에 가질 수 있는 전부이며, 파일이 없을 때 스스로 만들어 낸
무언가를 잠그는 대신 거부하게 만드는 것이기도 합니다. 하네스 자신의 수정은 그
inode를 유지합니다(파일 위에 `cp`, 그 위에 `tee -a`, 이름 바꾸기는 없음).
따라서 시작할 때 잠근 대상이 끝날 때 수정하고 있는 대상과 같습니다. 이 모드가
이미 요구하는 `sudo -n` 외에 이 잠금이 따로 요구하는 권한은 없습니다.

누가 쥐고 있는지는 아래의 실행 마커 디렉터리에 따로 기록되며 참고용입니다.
살아 있는 프로세스를 가리키는 라벨은 그 실행이 쥐고 있다는 뜻이고, 종료된
실행이 남긴 라벨은 없는 것으로 읽히며, 다른 사용자의 라벨은 이 실행이 읽을 수
없는 그 사용자의 디렉터리에 있습니다. 그럴 때 거부 메시지는 엉뚱한 실행을
지목하는 대신 그렇다고 말합니다. 그 라벨에서 읽어 무언가에 사용하는 값은
없습니다.

실행이 시작한 모든 자식이 그 서술자를 물려받으므로, 하네스는 자신이 띄우는
`bootroot-agent` 데몬에 대해 그것을 닫습니다(`9>&-`). 종료된 실행보다 오래
사는 데몬이 있으면 그 잠금을 계속 쥐고 있게 되기 때문입니다.

잠금을 잡으려면 `flock(1)`, `perl`, `python3` 가운데 호스트에 있는 것이 이
순서로 필요합니다. Linux 러너에는 셋 다 있고 macOS에는 뒤의 둘이 있습니다.
셋 다 없는 호스트에서는 `hosts` 모드가 거부되며, 무엇을 설치해야 하는지
알려 줍니다.

잠금 하나가 두 하네스를 모두 덮습니다. 둘 다 같은 호스트 이름 두 개를 추가하므로
로컬 실행과 원격 실행은 로컬 실행 두 개와 똑같이 서로를 덮어씁니다. 이 잠금은
...
이 제거는 모두 마커에서 읽어 낸 값으로 이루어지므로, 수거의 범위를 정하는
것은 결국 어떤 마커를 처리하느냐입니다. 마커는 두 값이 모두 같은 파생
네임스페이스 안에 있을 때만 인정됩니다. 파일 이름은 `e2e-local-` 또는
`e2e-remote-`로 시작하는 인스턴스여야 하고, `project` 항목은 그에 대응하는
`bootroot-e2e-local-` 또는 `bootroot-e2e-remote-`로 시작하는 Compose
프로젝트여야 합니다. 그 밖의 것 — 예전 명명 규칙으로 남은 마커, 손으로 만든
파일, 같은 경로를 골라 쓴 다른 도구가 남긴 절반짜리 기록 — 은 보고만 하고
발견한 그대로 둡니다. 수거하지도, 지우지도 않습니다. 기본 아이덴티티가 닿을
수 없는 이유는 그것을 가리키는 마커가 없어서가 아니라 구조적으로 표현할 수
없기 때문입니다. `bootroot`는 어떤 접두사로도 만들어지지 않으므로, 이름이
`bootroot`이고 `project=bootroot`에 죽은 pid를 기록한 파일이 있어도 실제
설치본의 컨테이너, 볼륨, 네트워크, `:latest` 응답기 이미지 어느 것도 수거의
표적이 될 수 없습니다. 같은 짝짓기 규칙은 마커를 쓸 때도 강제되므로, 표
바깥의 아이덴티티를 가진 실행은 이후 어떤 수거도 하지 못할 것을 설치하기
전에 멈춥니다.
...
고르는 셈이 됩니다. 네임스페이스 검사는 그 옆이 아니라 그 뒤에 섭니다.
소유권이 말해 주는 것은 다른 사용자가 마커를 쓰지 않았다는 사실이지, 이
하네스가 썼다는 사실이 아니기 때문입니다. 그 경로가 심볼릭 링크이면 따라가지 않고 곧바로 거부합니다.
...
`hosts` 모드 실행이 잠금을 쥐고 있다는 기록도 여기에 남습니다. 인스턴스가
가질 수 없는 이름(`hosts-lock-holder.label`)이라 수거가 그것을 실행으로 읽는
일은 없습니다. 그 기록이 머신 전체 경로가 아니라 이곳에 있는 이유는 잠금이
더는 그런 경로에 있지 않은 이유와 같습니다. 누구나 쓸 수 있는 디렉터리의
예측 가능한 이름은, 남이 심어 둔 링크가 가리키는 곳이 어디든 실행의 쓰기가
...
- 두 라이프사이클 스크립트를 통틀어 `hosts` 모드 실행은 한 번에 하나입니다.
  두 번째 실행은 첫 실행이 `/etc/hosts`에 쥔 `flock(2)`에서 거부됩니다
...
이 스크립트는 실행하는 동안 데몬이 공유하는 이미지 조회 태그를 바꿨다가
끝나면 되돌리므로 한 번에 하나만 실행됩니다. 같은 호스트에서 같은
사용자가 실행 중에 두 번째 실행을 시작하면 `docker` 명령을 하나도
실행하기 전에 거부됩니다. 앞선 실행이 끝난 뒤 다시 시작하세요. 잠금
파일은 `TMPDIR` 값과 관계없이
`/tmp/bootroot-deploy-smoke-<uid>/deploy-no-build-smoke.lock`이므로 서로
다른 셸이나 세션에서 시작한 실행도 같은 잠금을 두고 경합합니다. 그
디렉터리가 다른 사용자 소유이거나 다른 사용자가 쓸 수 있으면 스모크는
실행을 거부합니다. 다른 사용자나 같은 데몬을 쓰는 다른 호스트의 실행은
직렬화되지 않으므로 동시에 시작하지 마세요.
...
- 이유: `hosts` 케이스는 실행 중 호스트 머신의 `/etc/hosts`를
  ...
  필요합니다.
- 다만 이것만으로 매트릭스 전체를 실행할 수 있는 것은 아닙니다.
  `registrar-internal-init` 단계는 `bootroot init`을 root로 실행합니다.
  엔드포인트가 활성화된 `init`은 보호 대상 자격 증명 파일 다섯 개를
  `root:root`로 게시하며 그러지 못하면 어느 것도 게시하지 않으므로, 이 단계에는
  건너뛸 수 있는 비특권 형태가 없습니다. 아무것도 설치하기 전에 전제 조건
  블록에서 실패하며, `scripts/preflight/run-all.sh`도 함께 실패합니다. 비밀번호
  없는 sudo가 없는 머신에서는 이 시나리오를 아예 실행할 수 없습니다.
