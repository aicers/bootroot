...
오래된 잠금을 지울 때는 그 실행만 아는 이름으로 먼저 옮겨 놓고, 그렇게 손에
넣은 파일을 보고 판단합니다. 조금 전에 읽은 pid를 근거로 잠금 *경로*를 지우는
일은 없습니다. 두 실행이 같은 오래된 잠금에 도달하면 그 둘은 같지 않습니다.
그 사이에 두 번째 실행이 잠금을 지우고 가져갈 수 있고, 경로를 겨냥한 제거는
그때 살아 있는 실행의 잠금을 없앤 뒤 그 자리에 자기 것을 만들어, 두 실행이
모두 자기가 뮤텍스를 쥐었다고 믿으며 `/etc/hosts`를 고치게 됩니다. 이름 바꾸기는
원자적이므로 판단한 파일과 제거하는 파일이 하나입니다. 옮겨 놓고 보니 살아 있는
실행의 잠금이었다면 곧바로 되돌려 놓고 자신이 거부됩니다.
...
다만 *다른 사용자의* 종료된 실행이 남긴 잠금을 되찾으려면 그 경로가 치르는
대가가 하나 있고, 여기서는 그 값을 치릅니다. `/tmp`에는 sticky 비트가 있어
파일 소유자만 그 파일을 지울 수 있으므로, 평범한 `rm`은 종료된 실행의 잠금을
그 사용자에게만 되돌려 줍니다. 나머지 사용자에게는 그 사용자나 root가 손으로
파일을 지울 때까지 `hosts` 모드가 계속 거부되며, 그것이 바로 머신 전체 잠금이
없애려는 사용자별 직렬화입니다. 그래서 오래된 잠금 회수는 잠금을 옮기는 이름
바꾸기든 뒤따르는 삭제든 `sudo -n`으로 물러섭니다. 이 모드는 잠금에 닿기 전에
이미 그 권한을 요구하므로 추가로 요구하는 것은 없습니다. 그 회수를 허락하는
근거는 파일에서 읽은 pid이므로,
pid를 읽을 수 없는 잠금은 오래된 것이 아니라 누군가 쥐고 있는 것으로 취급합니다
(읽을 수 없는 pid는 죽은 pid가 아닙니다). 또한 그 잠금을 다음에 판단할 실행은
대개 그것을 쓴 실행이 아니므로, 잠금은 만들 때의 umask와 무관하게 누구나 읽을
수 있게 기록합니다.
