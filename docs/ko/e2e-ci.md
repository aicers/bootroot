...
활성화한 뒤, 권한 없는 `bootroot infra up`으로 그 위에 스택을 올립니다. 이
bring-up 자체가 하나의 확인입니다. 오버라이드에서 bind 원본을 읽어 그 저장소
디렉터리를 검사하는데, 마운트가 올라온 상태에서 그것은 *마운트된 파일시스템의
루트*이므로, 4단계에서 `audit_store_dir`에 `chmod 0700`을 다시 적용하지 않은
실행은 바로 다음에 오는 이 명령에서 거부됩니다. 이어서 컨테이너의
`/openbao/audit`이 마운트된 저장소의 bind mount인지, 두 하위 디렉터리와 감사
로그 자체가 마운트된 파일시스템 위에 있는지, 그리고 공용 확인
`assert_openbao_audit_log`가 — 그대로, 바뀐 것 없이 — 그 장치에 대해 통과하는지를
확인합니다. 자동 부팅 경로가 기대는 순서 관계는 설치된 파일이 아니라 systemd가
실제로 로드한 유닛에서 다시 읽습니다. `docker.service`가 마운트 유닛을
