...
2. **네트워크 도달성.** 원격 호스트는 다음에 접근 가능해야 합니다.
    ...
    세 엔드포인트 모두 제어 노드에서 기본적으로 `127.0.0.1`에
    바인딩됩니다. 이를 노출하는 관리형 `infra install` 바인드 플래그
...
래핑을 권장합니다. 원격 호스트는 디스크에 저장해야 하는 원본의 장기
유효 `secret_id` 대신 단회용 `wrap_token`을 받기 때문입니다.

`--no-wrap`은 어떤 전송 메커니즘을 선택하든 래핑을 비활성화하는
**독립적이고 선택적인** 옵션이며, 특정 옵션의 속성이 아닙니다. 이 옵션은
원격 호스트에 복사하여 저장해야 하는 원본 `secret_id`를 발급하므로
저장 시점(at-rest) 보안이 약해집니다. 유일하게 타당한 사용 근거는 재시도
시간 창입니다. 단회용 `wrap_token`은 TTL이 짧아(기본 30분) 느리거나
불안정한 전송 파이프라인에서 부트스트랩이 끝나기 전에 만료될 수 있는데,
이는 `--secret-id-wrap-ttl` 값을 늘려 완화할 수도 있습니다. 신규 배포에는
권장하지 않습니다. 아래 옵션 2~4의 예시는 마침 `--no-wrap` 형태(원본
`secret_id`와 개별 필드 플래그)를 보여주지만, 각 옵션은 래핑된
`bootstrap.json` 및 `--artifact`와도 똑같이 잘 동작합니다.
...
> 하며, 이때 `--artifact` 대신 개별 CLI 플래그를 전달할 수 있습니다
> (언래핑된 자격증명을 위한 대체 호출 형태).

### 옵션 2: systemd-credentials

> **참고:** 이 예시는 `--no-wrap` 형태(개별 CLI 플래그와 원본
> `secret_id` 파일을 systemd-credentials로 로드)를 보여줍니다. 래핑
> (기본값)도 systemd-credentials와 함께 동작합니다. `bootstrap.json`을
> 자격증명으로 전달하고 `--artifact`로 호출하세요. 트레이드오프는
> [전송 경계](#전송-경계)를 참고하세요.
...
> **참고:** 이 플레이북은 `--no-wrap` 형태(개별 CLI 플래그와 원본
> `secret_id` 파일 복사)를 보여줍니다. 래핑(기본값)도 Ansible과 함께
> 동작합니다. `secret_id` 대신 `bootstrap.json`을 복사하고 `--artifact`로
> 호출하세요.
...
> **참고:** 이 예시는 `--no-wrap` 형태(개별 CLI 플래그와 원본
> `secret_id`를 cloud-init 사용자 데이터에 직접 기록)를 보여줍니다.
> 래핑(기본값)도 cloud-init과 함께 동작합니다. `bootstrap.json`을
> `write_files`에 넣고 `--artifact`로 호출하세요.
...
없습니다. 원격 호스트의 `agent.toml`에 이 섹션이 없고 다시
부트스트랩하지 않으면 섹션이 계속 비어 있으며, 다음 번
`bootroot-remote bootstrap` 실행이 섹션을 채울 때까지 `force-reissue`
요청은 관측되지 않습니다.
...
state_path = "/etc/bootroot/bootroot-agent-state-giganto.json" # bootstrap이 기본 제공
```
...
그 파일을 두고 경합하며 서로의 fast-poll 진행 상태를 주기적으로
되돌립니다.
[한 호스트에서 서로 다른 서비스 여러 개 실행](#한-호스트에서-서로-다른-서비스-여러-개-실행)을 참고하세요.
...
원격 호스트에 동일한 `registration_id`를 가진 여러 `[[profiles]]`가 있을
경우(하나의 등록 키 아래에 여러 인스턴스를 운영하는 경우 등),
fast-poll 틱은 KV 버전을 소비된 것으로 표시하기 전에 일치하는 모든
프로파일에 갱신 트리거를 펼쳐 보냅니다. 따라서 단일 force-reissue가
모든 인스턴스를 회전합니다. `service_name`은 같지만 `registration_id`가
다른 프로파일은 서로 다른 등록이며, 각자의 KV 서브트리와 재발급 요청을
가지므로 함께 펼쳐지지 않습니다. 펼쳐 보낸 프로파일 중 하나라도 실패하면,
...
## 한 호스트에서 서로 다른 서비스 여러 개 실행

한 원격 호스트에서 서로 **다른** 서비스 여러 개를 실행할 수 있지만,
하나의 agent 구성을 공유할 수는 없습니다. 지원되는 구성은 **서비스마다
`bootroot-agent` 프로세스 하나와 agent 구성 하나**이며, 각각 자체
`[openbao]` 자격증명과 고유한 `state_path`를 가집니다.

서로 다른 서비스가 하나의 구성을 공유할 수 없는 이유는 두 가지입니다.

- `[openbao]` 섹션은 AppRole 자격증명(`role_id_path` /
  `secret_id_path`)을 **하나만** 담습니다. 두 번째 서비스를 같은
...
- 하나의 구성에 있는 여러 `[[profiles]]`는 *같은 서비스의 인스턴스*에
  대해서만 지원되며(위 참고), 서로 다른 서비스에는 해당하지 않습니다.
...
상태를 두고 경합하며 서로의 진행 상태를 주기적으로 되돌리기 때문입니다
(버전 게이팅 반복, 재발급 완료 추적 손실). 서비스별 구성을 각자의
디렉터리에 두어도 충돌을 피할 수 있습니다.
...
기본적으로 `docker-compose.yml`은 세 엔드포인트를 모두 `127.0.0.1`에만
게시하므로, 운영자가 명시적으로 선택하기 전까지는 어느 것도 원격
호스트에서 접근할 수 없습니다. 각 엔드포인트에는 노출 의도를
`state.json`에 기록하고 compose 오버라이드를 생성하는 관리형
`infra install` 바인드 플래그가 있습니다. `bootroot infra up`이 저장된
오버라이드를 다시 적용하므로 노출은 컨테이너 재생성 후에도 유지됩니다.

| 엔드포인트 | 바인드 플래그 | 동반 플래그 |
| --- | --- | --- |
| OpenBao API (`:8200`) | `--openbao-bind <IP>:8200` | `--openbao-tls-required`, `--openbao-bind-wildcard`, `--openbao-advertise-addr` |
| step-ca ACME 디렉터리 (`:9000`) | `--stepca-bind <IP>:9000` | `--stepca-bind-wildcard`, `--stepca-advertise-addr` |
| HTTP-01 관리자 API (`:8080`) | `--http01-admin-bind <IP>:8080` | `--http01-admin-tls-required`, `--http01-admin-bind-wildcard`, `--http01-admin-advertise-addr` |

`--stepca-bind`는 step-ca의 ACME 디렉터리(`:9000`)를 원격 노드에서
접근 가능하게 만드는 공식 지원 방법입니다.

```bash
bootroot infra install --stepca-bind 192.168.1.10:9000
```

- 이 플래그는 관리형 compose 오버라이드를
    `secrets/step-ca/docker-compose.stepca-exposed.yml`에 기록하고
    `state.json`에 `stepca_bind_addr`를 저장합니다. 플래그가 없으면
    기본 `127.0.0.1:9000` 게시가 그대로 유지됩니다.
- `bootroot init`이 step-ca를 구성하면서 저장된 오버라이드를
    적용하므로, 신규 설치 경로인 `infra install --stepca-bind` →
    `init`만으로 별도의 `infra up` 없이 `:9000`이 노출됩니다.
    `infra up`은 같은 오버라이드를 검증 후 다시 적용하므로 노출은
    컨테이너 재생성 후에도 유지됩니다.
- `0.0.0.0:9000`에 바인딩하려면 추가로 `--stepca-bind-wildcard`와
    `--stepca-advertise-addr <IP>:9000`이 필요합니다. 원격 노드는
    와일드카드에 연결할 수 없으므로 접근 가능한 구체적인 ACME 디렉터리
    주소가 기록되어야 합니다.
...
- 이 플래그는 번들 step-ca 서비스만 관리합니다. compose 파일에
    ...
    `--stepca-bind`를 거부하고, `infra up`은 그러한 compose 파일에
    대해 저장된 step-ca 오버라이드를 실패 대신 건너뜁니다.
- advertise 주소는 운영자 참고용으로만 기록됩니다. `service add`의
    `--agent-server`에 일치하는 ACME 디렉터리 URL을 직접 전달하세요
