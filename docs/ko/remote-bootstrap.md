...
등록된 서비스를 `local-file`에서 `remote-bootstrap`으로, 또는 반대로
바꾸는 작업은 제자리 업데이트가 아닙니다. `bootroot service remove
--service-name <service>`로 등록을 해제하고 저장된 OpenBao/KV 리소스를
정리한 뒤, 원하는 `--delivery-mode`로 `bootroot service add`를 다시
실행하세요. 기본적으로 remove는 cert/key와 기타 디스크 아티팩트를
보존합니다. 해당 로컬 아티팩트도 정리해야 할 때만 `--delete-artifacts`를
사용하세요.
