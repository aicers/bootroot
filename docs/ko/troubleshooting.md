...
`init`의 responder 점검은 실패 원인을 구분해서 보고하며, 각각 확인할
곳이 다릅니다.

- *"N초 동안 ... 연결하지 못했습니다"* — responder가 아예 응답하지 않은
  경우입니다. 아직 기동 중이거나, 실행되지 않았거나, 관리자 포트가
  게시되지 않은 상태입니다. 메시지에는 마지막 전송 오류가 함께 표시됩니다.
  `https://` responder에서는 TLS 신뢰/핀 검증 실패도 연결 실패로 나타나므로,
  기동이 느린 것이라고 단정하기 전에 이 원인을 먼저 확인하세요. 기본값
  60초보다 오래 걸리는 것뿐이라면 `--responder-ready-timeout-secs`를
  늘리세요. 이때 `--responder-timeout-secs`를 늘려도 소용이 없습니다.
  거부된 연결은 요청 타임아웃을 기다리지 않고 즉시 실패하기 때문입니다.
...
- *"Failed to build the HTTP-01 registration request"* — 요청 자체가 만들어
  ...
  `http://` 접두사가 빠진 것이 원인입니다. 이 경우에는 어느 타임아웃을
  늘려도 달라지지 않습니다.
...
- 먼저 해당 추가 명령의 요약을 확인하세요. `- 등록된 HTTP-01 DNS 별칭:`
  ...
  확인하세요)`는 당시 리스폰더가 내려가 있었다는 뜻입니다. 추가 자체는
  성공했으므로 `bootroot infra up`으로 별칭을 등록하면 됩니다.
