...
데몬이 쓰는 인증서 파일은 **리프 다음에 발급자 체인**을 담습니다. 아래
체인 요구 사항이 요구하는 형태이며, 발급자는 `trust.ca_bundle_path`에도
함께 병합됩니다. 리프는 첫 번째 PEM 블록에 그대로 남으므로, 리프만
필요한 읽는 쪽은 여전히 거기에서 찾을 수 있습니다.
