...
**rename은 가정하지 않고 확인합니다.** rename 직후, 그리고 시그널
*이전에* 패스는 세대를 다시 stat해서 마커가 기록한 동일성을 지니고
있는지 확인합니다. rename은 아이노드를 보존하므로 그래야 마땅하지만,
...
마커가 데몬 자신의 일반 파일이 아니거나 — 심볼릭 링크, FIFO, 또는 상한을
넘는 크기 — 마커가 이 장치 자신의 세대가 아닌 이름을 담고 있거나, 마커가
