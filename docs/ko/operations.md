...
범위**입니다. 정상 모델 — 직전 패스가 한 주기 안에 충분히 성공했고
OpenBao가 주기당 `W` 바이트를 추가한다 — 에서 장치는 패스 사이에
대략 `S × (N + 1) + W`(기본값 512 MiB + `W`)이고, 패스 내부 최고점은
대략 `S × (N + 2)`에 한 주기 분량 쓰기의 두 배를 더한 값(기본값
576 MiB)입니다. 여기서 `S`는 `openbao_audit_max_file_bytes`,
`N`은 `openbao_audit_max_retained_files`입니다. 최고점에
두 배 항이 있는 이유는, 스테이징 사본이 장치 자신의 디렉터리 안에
있어야 하기 때문입니다. 하드 링크는 파일 시스템을 넘지 못하고, 발행은
사본을 다시 복사하는 대신 제자리로 링크하기 때문입니다. 이는 기대되는
정상 상태와 최고점이며, 최악의 경우가 아닙니다.
...
**메커니즘과 손실 구간.** 활성 로그는 rename이나 unlink, 교체가 아니라
**복사 후 truncate**됩니다. 따라서 OpenBao가 쥐고 있는 경로와 파일
디스크립터는 계속 존재하고 쓰기 가능한 상태로 남으며, OpenBao는
재시작·재봉인되지도 시그널을 받지도 않습니다. 한 패스는 활성 로그의
안정적인 앞부분을 같은 디렉터리의 스테이징 파일로 복사하고, 마지막
완전한 레코드까지로 잘라내고, 모드 `0600`과 디렉터리 소유자를 부여하고,
플러시한 뒤 세대 이름으로 링크하고, 디렉터리를 플러시하고, 그런
다음에야 활성 로그를 0으로 truncate하고 이를 플러시합니다.
...
그 대가는 로테이션마다 생기는 작은 손실 구간입니다. 잘린 마지막
레코드와 복사가 진행되는 동안 OpenBao가 추가한 내용은 truncate가 통째로
없앱니다. 따라서 레코드는 세대 안에 온전히 들어 있거나 전부 그 구간
안에 있으며, 쪼개지거나 찢어지지 않습니다.

**크래시 잔여물.** truncate와 그 플러시 사이의 크래시는 세대는
