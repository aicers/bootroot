...
**프로덕션 요청 핸들러가 추가되기 전까지 활성화는 계속 지원되지
않습니다.** 이 빌드에는 버전이 지정된 레지스트라 페이로드 코덱이 포함되어
있지만 동사를 호출하는 핸들러는 등록되어 있지 않으므로
...
`server_cert_path`나 `server_key_path`가 없을 때, 둘 중 어느 쪽의 자재든
없거나 읽을 수 없거나 파싱할 수 없거나 키가 맞지 않거나 SAN이 맞지 않을 때,
인증서 파일이 발급자 체인 없이 리프만 담고 있거나 `trust.trusted_ca_sha256`에
핀으로 지정된 인증서까지 올라가지 못하는 체인을 담고 있거나 앵커가
만료됐거나 아직 유효하지 않거나 CA가 아닐 때, 그리고
`trust.ca_bundle_path`가 없거나 읽을 수 없거나 파싱할 수 없거나 핀에 맞는
인증서를 하나도 담고 있지 않을 때입니다.
...
**서버 자재는 대역 외로 공급합니다.** 이 리프에 대한 인증서 발급이 들어오기
전까지 bootroot는 그것을 만들지 않고, 자체 서명 대체물을 생성하지 않으며,
호스트의 일반 서비스 리프로 대체하지도 않습니다.

두 인증서 경로 모두 리로드할 수 없습니다. `enabled`, `server_cert_path`,
`server_key_path` 중 하나라도 바꾸는 `SIGHUP`은 바뀐 키를 밝히는 진단과 함께
거부되고, 실행 중인 데몬은 그대로 남습니다. 실행 중인 데몬 아래에서 바뀔 수
있는 것은 그 경로들의 *내용*뿐입니다.
