...
효과가 납니다. 같은 이유로 활성화된 엔드포인트는 그 판정이 핸들러가 더는
쓰지 않는 저장소를 가리키게 만드는 `SIGHUP`을 거부합니다. 어느 방향으로든
`audit_store_enforcement`이 바뀌거나, enforcement가 `filesystem`인 채로
`audit_store_dir`이 바뀌는 경우입니다. 거부는 데몬의 언어로, 키와 두 값을
함께 적어 로그에 남고, 실행 중인 데몬은 시작할 때의 설정을 그대로
제공합니다. 변경을 적용하려면 서비스를 재시작하세요.
`audit_store_enforcement = "directory"`로 실행 중인 엔드포인트는 판정을
내리지 않았으므로, `directory` 모드를 유지한 채 `audit_store_dir`만 옮기는
리로드는 다른 `[registrar]` 변경과 똑같이 적용됩니다. 다음 실행 단위가 옮겨진
저장소를 열고, 마운트 메타데이터를 읽지 않으며, 마운트에 관해 아무것도
기록하지 않습니다. `audit_record_dir`은 저장소 안에 있어야 하므로, 파일에
그 키를 적어 두었다면 같은 편집에서 함께 옮기세요. 키를 생략한 파일은
저장소에서 파생되므로 추가 변경이 필요 없습니다. 엔드포인트가 비활성이면 이
가운데 어느 것도 실행되지 않습니다.
...
**다섯 가지 상태는 결과를 올리면서 모든 명령을 거둬 갑니다.** 그 각각에서
...
- **보관 경로가 디렉터리가 아닌 경우.** 복사는
  ...
  측정·순회·렌더링하기 전에 거부합니다. 활성화도 마운트 상태와 무관하게
  복사와 함께 렌더링하지 않습니다. 그 경로의 것은 결코 복사될 수 없으므로,
  그것을 위해 예약량을 마운트하는 것은 얻는 것 없이 호스트만 바꾸는
  일입니다. 되돌리기는 그대로 제시됩니다.
- **`<audit_store_dir>`에 예약량이 아닌 것이 마운트되어 있는 경우** —
  tmpfs, 바인드 마운트, 또는 다른 파일을 받치는 루프 장치가 그렇습니다.
  이를 "예약량이 마운트되지 않음"으로 보고하고 활성화를 렌더링하면 그
  위에 예약량이 겹쳐 마운트되고, 복사는 저장소의 레코드를 그 안에 쓰며,
  되돌리기의 `umount`는 이 이전이 놓지 않은 파일 시스템을 내립니다.
  대신 무엇이 마운트되어 있는지와 함께 이름을 밝히고, 셋 중 어느 것도
  렌더링하지 않습니다. 두 기록자를 멈추고 그것을 직접 마운트 해제하거나
  치운 뒤 명령을 다시 실행하세요. bootroot는 이 절차의 어디에서도
  아무것도 마운트 해제하지 않습니다.
- **이전이 열려 있는 동안 `<audit_store_dir>.migrated`가 이미 있는
  경우.** **migration incomplete**를 지우는 것은 마무리 이름
  바꾸기인데, 그것은 이미 있는 경로를 대상으로 삼을 수 없습니다.
  그래서 끝단이 결코 실행될 수 없는 절차를 시작하게 하는 대신 복사
  자체를 거부합니다. 그러지 않으면 두 기록자를 멈춘 채 유지 창 전체를
  쓰고도 이전이 여전히 열린 상태로 끝나게 됩니다. 활성화도 마운트
  상태와 무관하게 함께 거둬 갑니다. 그 경로가 서 있는 한 어떤 회차도
  복사에 이르지 못하기 때문입니다. 되돌리기는 그대로 제시됩니다.
  복원하는 이름 바꾸기의 대상은 `<audit_store_dir>`이고, 막힌 마무리
  이름 바꾸기는 그에 대해 아무것도 말하지 않기 때문입니다. 이미 있는
  `<audit_store_dir>.migrated` 아래에 아무것도 마운트되어 있지 않은지
  확인하고 직접 치우거나 지운 뒤 명령을 다시 실행하세요. 다음 회차가
  전체 절차를 렌더링합니다.
...
1. 두 기록자를 모두 멈춥니다.
   ...
   기록자를 멈춘 바로 그 배포를 상대로 진행됩니다. 실행에 어떤 형태로
   주어졌든 `--compose-file`과 `--agent-config`는 모두 **절대 경로**로
   렌더링됩니다. 7단계의 검증은 `<audit_store_dir>` 안에서 끝나므로,
   상대 경로 `docker-compose.yml`을 그대로 9단계까지 들고 가면 배포가
   있는 곳이 아니라 저장소 아래에서 그 파일을 찾게 됩니다.
...
3. `bootroot infra up --compose-file <compose file> --agent-config
   ...
   기록자를 멈추는 단계도 두 명령을 모두 갖춘 채 그 목록에 나타납니다.
   1단계가 이미 그것을 충족했습니다.
...
비교합니다. 그 크기는 아래에 있는 모든 것뿐 아니라 보관 디렉터리 자신도
셉니다 — 대상 쪽 루트 디렉터리도 같은 항목들을 담느라 커져야 하기
때문입니다. 그래서 이 값은 `du -s -x --block-size=1`이 보고하는 값과
정확히 같으며, [교체 절차](#changing-the-reserve)에서 운영자가 같은 판단을
위해 손으로 구하는 값이기도 합니다. 세 값을 모두 보고하므로 "복사를 마저 하라"와 "이건 절대 들어가지
...
실행마다 다시 판정합니다. 둘 다 마운트 상태보다 앞서 판정되므로 예약량이
아직 마운트되지 않은 상태에서도 나오는데, 그때는 **활성화도 복사와 함께
빠집니다**. 활성화는 마운트를 올리는 단계이고, 그 옆에 렌더링되는 롤백은
저장소가 이미 마운트 지점일 때만 `umount`를 담습니다. 따라서 둘을 함께
담은 목록은 마운트를 만들어 놓고 그 마운트에 대한 `rmdir`에서 멈춰,
되돌리는 이름 바꾸기까지 가지 못합니다. 보고가 지목한 것을 치우고 명령을
다시 실행하십시오. 보관 트리가 깨끗해지면 다음 실행이 평소대로 활성화를
렌더링합니다. 예약량이 이미 마운트되어 있다면 활성화는 그대로 남습니다.
그 목록에는 롤백이 전제한 마운트를 새로 만드는 것이 없기 때문입니다.
...
- **되돌리기**는 보관 디렉터리가 아직 `.pre-mount` 이름을 지니고 있는
  ...
  `umount`은 저장소가 마운트 지점일 때, 곧 4단계부터만 렌더링됩니다. 2단계와 4단계 사이에는 마운트가 없는 채로 이관이 이미
  열려 있고, 그때 렌더링되는 되돌리기는 `rmdir`과 이름 되돌리기뿐입니다.
  그 앞에 놓인 `umount`은 0이 아닌 상태로 끝나 순서대로 실행하는 흐름을
  둘 중 어느 것에도 닿기 전에 멈춰 세울 것입니다.

  **모든 줄은 보관 경로를 조건으로 삼습니다.** 되돌리기가 멱등인 이유가
  그것입니다. 보관 경로는 이전이 아직 열려 있음을 뜻하는 상태이므로,
  되돌리기가 한 번 끝났거나 마무리 이름 바꾸기가 끝난 뒤에는 같은 세 줄이
  모두 건너뛰며 0으로 끝납니다. 방금 레코드를 되돌려 놓은 저장소에
  `rmdir`을 시도하다 멈추지 않습니다. `rmdir`에는 마운트 지점이 존재하는지에
  대한 조건이 하나 더 붙습니다. 2단계와 4단계 사이에는
...
#### 예약량이 차오르는 것 지켜보기 {#watching-the-reserve-fill}

상한은 스스로 곧 도달한다고 알려 주지 않습니다. 그래서 엔드포인트가
활성인 데몬은 이미 돌고 있는 회전 루프의 유지보수 틱에서 — 1분에 한 번 —
저장소를 측정하고, 그 결과를 모든 응답의
`registrar_health.audit_capacity` 멤버로 보고합니다. 이를 위해 새로
잡히는 스케줄러는 없으며, 요청 경로에서는 어떤 측정도 수행되지 않습니다.

각 틱은 `audit_store_dir`에 대해 두 숫자를 보고합니다. 저장소의 사용량,
그리고 뒤를 받치는 파일 시스템이 비특권 기록자에게 아직 내줄 수 있는
바이트입니다. `filesystem` 강제에서는 저장소가 곧 하나의 파일
시스템이므로 둘 다 저장소 루트에 대한 한 번의 `statvfs`에서 나옵니다.
`directory` 강제에서는 그 호출이 저장소가 아니라 호스트를 측정하게 되므로,
사용량은 대신 저장소를 순회하며 각 항목의 **할당된** 블록을 더해 구합니다.
경보가 읽는 여유 공간은 설정된 예산의 잔여분과 장치의 가용 바이트 중 작은
쪽입니다.
...
`min`이 핵심입니다. 장치의 여유 공간보다 큰 예약량은 예약량이 아니며,
경보는 둘 중 먼저 걸리는 쪽에서 울려야 합니다. `headroom_bytes`는
부호 있는 값입니다. 예약량을 넘어선 저장소는 가장 건강해 보이는 0이 아니라
음수 여유 공간을 보고합니다.

`state`는 네 값 중 하나이고, 그 옆의 `enforcement`는 그 뒤에 선 예약량이
커널이 강제하는 것인지 설정된 추정치인지 말해 줍니다. 같은 여유 공간
숫자라도 두 모드에서 필요한 대응이 다르기 때문입니다.

| `state` | 의미 |
| --- | --- |
| `unknown` | 아직 성공한 측정이 없음 |
| `ok` | 여유 공간이 `audit_store_low_water_bytes`보다 큼 |
| `low_water` | 경보가 켜져 있고 예약량은 아직 소진되지 않음 |
| `exhausted` | 여유 공간이 0 이하, 즉 예약량에 닿았거나 넘어섬 |

`unknown`은 `used_bytes`, `headroom_bytes`, `measured_at`이 빠지는 유일한
상태이며, "아직 재지 못했다"가 건강하다고 보고되는 일이 없도록
존재합니다. `exhausted`는 음수만이 아니라 0인 여유 공간도 포함합니다.
0에서는 다음 쓰기가 이미 예약량을 넘어서기 때문입니다.

경보는 정확히 `audit_store_low_water_bytes`에서 **켜집니다**. 설정한
숫자가 "512 MiB 남았을 때 경보"로 읽히도록 한 것이지, "그보다 조금 아래"가
아닙니다. 냉시작에서는 그보다 1바이트 위면 꺼져 있습니다. 다시 올라올
때는 히스테리시스 여백이 붙잡아 두므로, 임계값 언저리를 오가는 저장소가
콘솔을 깜빡이게 하지 않습니다. 경보는
`audit_store_low_water_bytes + margin`에서만 해제되며, 여백은
`audit_store_low_water_bytes / 10`과 1 MiB 중 큰 쪽입니다. 이 관문은 경보
상태에서 *돌아 나오는* 경우에만 적용됩니다. 한 틱 만에 해제 임계값에
도달한 저장소는 `low_water`에 머물지 않고 곧바로 `ok`가 됩니다.

같은 틱이 레코드 저장소 자체의 세 신호 — `bootroot status
--agent-config`가 호스트에서 출력하는 `intent_without_outcome`,
`malformed_records`, `retention_shortfall` 값 — 을 같은 30일 조회 창으로
읽어 용량 숫자 옆에 함께 실어 보냅니다. 콘솔이 호스트의 운영자와 같은 것을
보게 하기 위해서입니다. 이 네 멤버는 `audit_record_dir`을 읽고, 용량
숫자는 `audit_store_dir` 전체를 읽습니다.

**두 개의 타임스탬프와 그 신선도의 상한.** `measured_at`은 마지막으로
성공한 용량 측정, `records_measured_at`은 마지막으로 성공한 레코드
스캔입니다. 둘이 따로인 이유는 두 작업이 독립적으로 실패하기 때문입니다.
읽을 수 없는 레코드 저장소는 스캔을 실패시키지만 `statvfs`는 여전히
성공합니다. 실패는 이전 값과 이전 타임스탬프를 그대로 남기고 초기화하지
않습니다. 경보를 올리는 장치의 실패 뒤로 경보가 숨는 일이 없어야 하기
때문입니다. 둘 다 UTC RFC 3339이며, 건강한 데몬에서는 어느 쪽도 1분보다
더 오래되지 않습니다. 이들을 찍는 유지보수 틱이 1분에 한 번 돌기
때문입니다. 그보다 오래된 타임스탬프는 측정이나 스캔이 성공을 멈췄다는
신호이므로, 옆의 숫자를 믿기 전에 먼저 읽으십시오.

경보는 성공 응답만이 아니라 **거부** 응답에도 실립니다. 이 저장소는
fail-closed 통제이므로 차오를수록 호출이 거부됩니다. 성공에만 실리는
신호라면 경보가 존재하는 바로 그 상태에서 경보를 나르기를 멈출 것입니다.

`low_water`나 `exhausted`인 저장소는 저절로 낫지 않습니다. 필요한 것은
위의 크기 산정이며, 무한정 자라는 기록자가 OpenBao의 파일 감사 장치이므로
그 장치의 세대에 대한 결정입니다. 그리고 예약량이 애초에 약속하지 않은
것도 함께 기억해 두십시오. 예약량은 피해 범위를 감사 산출물로 한정할
뿐입니다. 예약량이 가득 차면 OpenBao는 여전히 서비스를 멈춥니다. 파일 감사
장치가 필수이고, 감사할 수 없는 요청은 실패시키기 때문입니다.
...
**호출자는 자신이 왜 거부됐는지 알 수 없지만, 자신의 문법적으로 유효한
알 수 없는 동사가 거부됐다는 사실만은 알 수 있습니다.** 그 한 경우에는
프레임된 본문 `unrecognized-operation`을 반환하므로, 호출자는 의도적인
거부를 무관한 빈 교환과 구별할 수 있습니다. 실패한 핸드셰이크는 호출자에게
TLS 경보로만 전달되고, 그 밖의 모든 핸드셰이크 이후 거부는 바이트 없는
깨끗한 종료로 전달됩니다. 모든 로그 줄에는 그 연결의 나머지 줄들과 이어
볼 수 있게 해 주는 연결 id와 `reason` 필드가 들어 있습니다.
...
#### 두 리프 지켜보기, 그리고 만료 읽기 {#watching-the-two-leaves-and-reading-a-lapse}

갱신이 멈춰도 무언가 만료되기 전까지는 아무 소리가 나지 않습니다. 그래서
엔드포인트는 헬스 스냅숏을 싣는 모든 응답에서 — mint 성공, deregister 성공,
거부 모두 — 두 리프를 함께 보고합니다. `registrar_health.certificates`는 정확히
두 개의 항목을 담은 배열이며, 순서는 언제나 `registrar_client`가 먼저,
`endpoint_server`가 그다음입니다.
...
`remaining_seconds`는 부호 있는 값이며, 남은 시간을 초 단위로 **내림**한
값입니다. 만료 시점 정각에는 `0`이고 그 순간의 리프는 아직 유효합니다. 만료
직후부터는 음수가 되며, 만료된 지 1초가 채 지나지 않았다면 `-1`입니다. 따라서
이미 만료된 리프는 마지막 정상 시점으로 오해할 수 있는 `0`이 아니라 음수로
읽힙니다.

`last_renewal_outcome`은 `never_attempted`, `succeeded`, `failed` 중
하나이고, `last_renewal_at`은 뒤의 두 경우에만 실리고 첫 번째 경우에는
빠집니다. 두 항목 모두 엔드포인트가 첫 요청을 처리하기 전에 이미 채워져
있습니다. 갱신 루프를 준비하는 단계에서 디스크에 이미 있는 각 인증서를 읽어
그 `notAfter`를 `never_attempted`와 함께, 시각 없이 기록하기 때문입니다. 이
시드는 OpenBao에도 CA에도 아무것도 요청하지 않으며 갱신 시도가 아닙니다.
그러므로 첫 응답에서 두 리프가 모두 `never_attempted`인 것은 막 기동한 데몬의
정상적인 모습이지, 갱신이 망가졌다는 증거가 아닙니다.

**주목해야 할 조합은 `remaining_seconds`가 아직 양수인데
`last_renewal_outcome`이 `failed`인 상태입니다.** 리프는 유효하고 호출자는
아무 영향을 받지 않으며, 지금부터 `not_after`까지의 구간이 이 문제를 조용히
고칠 수 있는 유일한 창입니다. 여기에 알림을 걸고, 무엇을 고쳐야 하는지는
저널에 남은 데몬 자신의 갱신 오류에서 읽으세요.

오래 떠 있는 데몬에서 `never_attempted`가 계속 보이는 것 자체는 문제가
아닙니다. 리드 타임 안에 들어오지 않았고 `[trust] ca_bundle_path`까지 체인도
그대로인 리프에 대해서는 패스가 아무것도 시도하지 않으므로, 리프가 실제로
갱신 대상이 될 때까지 결과는 시드 상태 그대로 남습니다. 갱신이 멈췄다는 신호는
`remaining_seconds`가 리드 타임 아래로 내려갔는데도 결과가 여전히
`never_attempted`인 상태입니다.

요청을 처리하는 경로는 인증서 파일을 읽지 않습니다. 데몬이 감사 저장소 용량
신호와 같은 유지보수 주기에 두 항목을 공유 스냅숏으로 복사하므로, 값은 요청
시점이 아니라 그 주기에 맞춰 움직입니다.

##### 리프가 이미 만료됐을 때 {#when-a-leaf-has-already-lapsed}

만료된 리프는 어느 쪽이 만료됐는지를 밝히는 타입이 있는 오류로 호출자에게
전달됩니다. 일반적인 연결 오류나 핸드셰이크 오류로 뭉뚱그려지지 않고, 재시도로
해결되는 오류로도 분류되지 않습니다.

- **`registrar_client`** — 호출자 자신의 인증서가 `notAfter`를 지났습니다.
  클라이언트는 다이얼마다 자재를 읽는 단계에서 이를 감지하고 소켓을 열기 전에
  반환하므로, 연결 시도 자체가 일어나지 않습니다. 이 오류는 데몬이 떠 있지
  않은 경우보다 우선합니다. 리프가 만료돼 있으면 아무도 듣고 있지 않더라도
  만료 오류가 돌아옵니다.
- **`endpoint_server`** — 엔드포인트의 인증서가 `notAfter`를 지났고, TLS 검증이
  그렇게 판정했습니다. 연결이 핸드셰이크 단계까지 간 뒤에만 보고됩니다.

둘 다 옆에 있는 재시도 가능한 연결 실패들과 구분되며, 어느 쪽도 호출자가
고칠 수 있는 것이 아닙니다. 핀에 없는 앵커, 잘못된 SAN, 이어지지 않는 체인 등
그 밖의 모든 핸드셰이크 판정은 이전과 똑같이 동작합니다.

**복구 방법은 리프가 이미 만료됐는지, 아니면 아직 유효한데 갱신만 실패하고
있는지에 따라 달라지며, 둘은 서로 다른 것을 필요로 합니다.**

- **리프가 이미 만료됐고 데몬이 멈춰 있는 경우.** 데몬을 시작하세요. 기동 시점
  발급은 엔드포인트가 자재를 로드하기 전에 돌면서 쓸 수 없는 쌍을 재발급하므로,
  리프는 엔드포인트가 서비스를 시작하기 *전에* 복구됩니다. `systemctl start
  bootroot-agent` 한 번이면 표면이 유효한 인증서와 함께 돌아옵니다.
- **리프는 아직 유효한데 마지막 갱신이 실패한 경우.** 무엇을 시작해도 달라지지
  않습니다. 데몬은 이미 떠 있고 디스크의 인증서는 아직 쓸 수 있으므로, 기동
  시점 발급이 재발급할 것이 없습니다. 이를 고치는 것은 *다음* 갱신 패스가
  성공하는 것뿐이고, 그러려면 실패가 지목한 원인을 없애야 합니다. OpenBao에
  닿지 않거나, 내부 자격 증명이 만료되었거나 밀려났거나, CA가 발급을
  거부하거나, 신뢰 앵커 로테이션 뒤 핀 파일이 지목한 앵커까지 대체본의 체인이
  이어지지 않는 경우 등입니다. 그것을 고치면 실행 중인 데몬의 다음 패스가
  대체본을 발행하고, 재시작 없이 활성 TLS 설정을 교체합니다.

어느 쪽이든 호스트를 다시 프로비저닝하지 **마세요.** 두 리프 모두 데몬이 스스로
발급하고 갱신하는 인증서이며, 만료는 그 유지보수가 멈췄다는 뜻이지 배포가
잘못됐다는 뜻이 아닙니다. 배포를 다시 초기화하면 타이머 하나를 고치려고
멀쩡한 PKI와 등록된 모든 서비스 신원, 감사 기록을 버리게 됩니다.
