...
- 초기 설정 시 서비스별로 `bootroot-remote bootstrap`을 1회 실행합니다.
...
- **한 호스트에서 서로 다른 서비스 여러 개**: 서비스마다 `bootroot-agent`
  ...
  bootstrap은 기본 제공하는 `state_path` basename을 서비스 이름 기반으로
  설정하므로 서비스별 구성이 fast-poll 상태 파일 충돌 없이 한 디렉터리를
  공유할 수 있으며, 두 sibling 구성이 여전히 같은 `state_path`로 해석되면
  경고합니다. `docs/ko/remote-bootstrap.md`를 참고하세요.
- 점검 순서:
...
AppRole 로그인을 수행하며, `remote-bootstrap` 서비스에 대해서는 OpenBao
Agent 사이드카 아티팩트가 더 이상 생성되지 않습니다.
