...
**실행 시간 거부.** 엔드포인트가 활성화된 `filesystem` 배포에서는 데몬이
시작할 때 `audit_store_dir`이 마운트 지점인지 확인합니다. 그렇지 않으면
데몬은 저장소 경로와 예상 마운트 유닛을 한 번 로그에 남기고, 인증서 갱신과
fast-poll 작업은 계속 수행하며, 레지스트라 연결에는 영구
`registrar_unavailable` 거부(`audit_unwritable`)만 응답합니다. 활성화된
소켓에 호출자를 기다리게 두지 않으며 재시도로 이 상태가 고쳐지지도
않습니다. 확인은 데몬 시작 시에만 하므로, 나중에 저장소를 마운트했다면
다음 시작부터 적용됩니다. 엔드포인트가 비활성화되었거나 enforcement가
`directory`이면 이 확인은 전혀 실행되지 않습니다.

거부 응답마다 새 `request_id`가 생성됩니다. 감사 저장소를 쓸 수 없어서
거부하는 것이므로 그 id에는 감사 레코드가 없습니다. 대신 데몬은 그 id,
호출자, 작업, 저장소 경로 및 예상 마운트 유닛을 담은 정보 로그 한 줄을
남깁니다. 호출자가 그 id를 보고하면, 존재하지 않는 저장소 레코드를 찾지
말고 journal에서 거부를 찾으세요.

엔드포인트가 활성화된 에이전트를 시작하기 전에 `BOOTROOT_LANG=ko`를 설정하면
이 두 journal 진단이 한국어로 출력됩니다. 기본값은 영어입니다.
...
3. **데몬이 실행 중에 마운트를 잃는 것은 감지하지 않습니다.** 데몬의 마운트
   확인은 시작할 때만 하므로, 그 뒤의 손실은 다음 데몬 시작부터 적용됩니다.
