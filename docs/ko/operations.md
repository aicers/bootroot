...
셉니다. 합계가 아니라 둘을 따로 읽으세요. `predecision_refusal`이 오르는
것은 누군가 잘못된 입력을 대량으로 밀어 넣고 있다는 뜻이며 그 호출자들은
여전히 진짜 답을 받고 있습니다. `admission`이 오르는 것은 정상 트래픽이
막히고 있다는 뜻이며 진행 중인 브링업이 지연되고 있을 수 있습니다.
