...
머신에서 `bootroot-remote apply-secret-id`를 실행하면 회전된 `secret_id`가
에이전트의 로컬 파일에 기록됩니다. `remote-bootstrap` 흐름에서 원격
`bootroot-agent`는 fast-poll 루프 안에서 AppRole로 OpenBao에 직접
재인증하며, `bootroot-remote bootstrap`이 프로비저닝한 `[openbao]` 블록의
`role_id_path`/`secret_id_path`가 가리키는 파일에서 `role_id`/`secret_id`를
읽어 다음 AppRole 로그인 시 회전된 `secret_id`를 반영합니다.
