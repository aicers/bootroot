...
`enabled = true`일 때 네 경로는 모두 필수이며, 서로 다른 네 개의 파일을
가리켜야 합니다. 설정되지 않은 경로도, 둘이 같은 파일을 가리키는 경우도
무엇을 읽거나 요청하기도 전에 설정 검증 시점에 거부됩니다. 두 쌍은 차례로
발급되므로 경로를 공유하면 마지막에 쓰인 것만 남습니다. 엔드포인트는
문제없이 로드되는 리프를 제시하며 기동하는데, 레지스트라는 바로 그 파일을
자기 클라이언트 자격 증명으로 읽게 됩니다.

데몬이 쓰는 인증서 파일은 **리프 다음에 CA가 돌려준 발급자 체인**을
담습니다. 아래 체인 요구 사항이 요구하는 형태이며, 그 발급자는
`trust.ca_bundle_path`에도 함께 병합됩니다. 리프는 첫 번째 PEM 블록에
그대로 남으므로, 리프만 필요한 읽는 쪽은 여전히 거기에서 찾을 수
있습니다.

CA가 **발급자를 하나도 붙이지 않고** 리프만 돌려준 경우가 둘 중 어느
것도 남기지 않는 유일한 경우입니다. 발급은 경고를 남기고
`trust.ca_bundle_path`를 그대로 둔 채 리프만 게시하며, 그 파일에 대해
아래 체인 요구 사항이 기동을 거부합니다. 파일을 사람이 놓아 둔 것으로
넘겨짚지 말고 기동 진단을 읽으십시오. 다시 발급해 복구되는 것은 사용할
수 없는 여덟 가지 상태이고, 이 경우는 거기에 들지 않습니다.
...
- **`trust.ca_bundle_path`를 읽을 수 없는 경우.** *게시 이전에*
  실패합니다. 데몬이 내용을 확인할 수 없는 번들을 덮어쓰기를 거부하므로
  리프는 쓰이지 않고, 거부가 번들을 밝히며, 번들은 바이트 단위로 그대로
  남습니다. *없는* 번들과 *읽을 수는 있으나 파싱되지 않는* 번들은 이
  목록에 없으며 복구됩니다. 아웃바운드 ACME 연결이 자신의 TLS를 같은
  파일에 앵커하므로, 데몬은 먼저 bootroot-internal 자격 증명의 CA
  번들에서 그 파일을 복구하고 — `trust.trusted_ca_sha256`이 핀으로 지정한
  앵커만 남깁니다 — 그렇게 했음을 로그로 남긴 뒤 발급합니다. 파싱되는
  번들은 이 복구가 결코 덮어쓰지 않습니다.
...
않으며, 읽은 값을 디스크에 되쓰지도 않습니다. 읽기 실패는 실패한 읽기
**그리고** 그로 인해 발급되지 못한 자재 경로를 밝히면서 기동을 거부합니다.
`bootroot init`은 모든 배포에서 `bootroot/agent/eab`를 쓰며 — 등록할 EAB가
없었던 경우에는 `kid`와 `hmac`이 빈 값인 형태입니다 — 그래서 EAB가 없는
배포는 바인딩 없이 발급하고, 레코드가 **없는** 경우는 실패한 읽기로 거부합니다.
신뢰 회전 구간에서 발급이 필요한 기동은 자격 증명 자신의 복구 진단과 함께
거부됩니다. 회전을 끝내는 것이 해결책입니다.
...
중 하나라도 없을 때, 그 넷 중 둘이 같은 파일을 가리킬 때, 발급 자체가
실패했을 때, 인증서 파일이 발급자 체인
