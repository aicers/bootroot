...
- 게시는 되돌릴 수 있습니다. 첫 파일을 교체하기 전에 대상 세 개의 내용과 권한
  ...
  내용으로, 원래 모드로, 원래 uid와 gid 소유로 되돌립니다. 손으로 좁혀 둔
  인증서가 이 명령이 쓰는 `0644`로 돌아오거나, 다른 계정이 소유하던 키가 실패한
  실행의 소유로 돌아오지 않습니다. 최초 프로비저닝이라 되돌릴 내용이 없으면
  실패한 실행이 만든 파일을 제거합니다. 권한이 없어 `chown`이 거부된 경우를
  포함해 되돌리기까지 실패하면, 직접 확인해야 할 파일 목록을 오류에 담아
  알립니다
