...
출력 두 파일은 `0600`, 발급된 인증서와 CA 번들은 `0644`입니다. 그 밖의 파일은
대상에 이미 있는 모드를 그대로 씁니다. 손으로 좁혀 둔 파일은 이후 모든 쓰기에서도
좁은 채로 남고, 새로 만들 때는 프로세스 umask가 허용하는 값 — 일반적인 `open(2)`가
`0666`에 umask를 적용해 내놓는 바로 그 값 — 을 받습니다. `state.json`,
`ca.json`, `openbao.hcl`, compose 오버라이드, `init`이 롤백에서 되돌리는 파일이
여기에 해당하며, `umask 077`인 호스트에서는 `0600`으로, 기본값인 `umask 022`
에서는 `0644`로 만들어집니다.
