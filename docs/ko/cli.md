...
서비스 등록을 해제하고 제어 플레인 리소스를 정리합니다. 서비스의
`--delivery-mode`를 바꾸는 지원 경로는 이 명령으로 제거한 뒤 원하는 전달
모드로 `bootroot service add`를 다시 실행하는 것입니다.

### 입력

- `--service-name`: 서비스 이름 식별자
- `--dry-run`: OpenBao, DNS 별칭, 파일, `state.json`을 변경하지 않고
  정리 계획만 출력
- `--yes` / `--force`: 확인 프롬프트 생략
- `--delete-artifacts`: 해당 서비스의 bootroot 관리 디스크 아티팩트도
  제거합니다. 이 플래그가 없으면 cert/key 파일과 로컬 아티팩트는 보존됩니다.
  이 플래그를 사용해도 저장된 `agent.toml` 파일 자체는 삭제하지 않고,
  bootroot 관리 프로필 블록만 제자리에서 제거합니다.
- dry-run이 아닌 정리에는 OpenBao 런타임 인증 플래그(`--root-token`,
  `--root-token-file`, `--auth-mode approle`, `--approle-role-id`,
  `--approle-secret-id` 및 파일/환경 변수 변형)가 필요합니다.

### 동작

- 저장된 AppRole 이름, 저장된 policy 이름, 정확한 서비스별 KV 경로,
  선택적 아티팩트 정리 작업을 포함한 전체 계획을 먼저 출력합니다.
- `state.json`에 저장된 role/policy 이름을 사용해 OpenBao 리소스를 먼저
  삭제합니다.
- `service add`가 기록한 정확한 서비스별 KV 경로만 개별 삭제합니다.
- 서비스 제거 후 HTTP-01 responder 별칭을 다시 맞춥니다. 마지막 별칭 보유
  서비스를 제거해 결과 별칭 집합이 비어도 stale 별칭을 남기지 않습니다.
- `state.json`의 서비스 항목은 마지막에 제거합니다. 원격 정리가 실패하면
  저장된 이름으로 재실행할 수 있도록 state 항목을 유지합니다.
