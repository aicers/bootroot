...
- 게시는 되돌릴 수 있습니다. 첫 파일을 교체하기 전에 대상 세 개의 내용과 권한
  ...
  알립니다
- 같은 경로에 동시에 게시하는 두 실행은 서로 직렬화됩니다. 각 실행은 기록 대상
  디렉터리마다 `.bootroot-registrar-publish.lock` 파일에 배타적 `flock`을 걸고,
  대상을 읽어 두기 전부터 마지막 기록이 끝날 때까지 유지합니다. 따라서 뒤에 온
  실행은 기다렸다가 앞선 실행의 결과 위에 세 파일을 통째로 게시하며, 앞선 실행의
  중간에 끼어들지 않습니다. 잠금 파일은 내용이 없고 `0600`으로 만들어지며 이후
  지우지 않습니다. 잠금은 프로세스가 끝나면 커널이 해제합니다. 게시에 앞선 발급은
  직렬화하지 않습니다. 발급은 프로세스마다 이름이 다른 스테이징 디렉터리에만
  기록하기 때문입니다
