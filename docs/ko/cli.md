...
- **`agent.toml`, 발급된 인증서와 키, CA 번들, 모든 자격 증명은 경로 자체에
  게시되며**, 그 자리에 있던 링크는 일반 파일로 대체됩니다. 이들은 이미 여러
  릴리스 전부터 rename으로 게시되어 왔으므로 해당 경로의 링크는 파일을 만드는
  명령을 견딘 적이 없습니다. 자격 증명의 경우 링크를 따라가는 것은 링크를 심을
