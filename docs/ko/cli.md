...
- 게시는 되돌릴 수 있습니다. 첫 파일을 교체하기 전에 대상 세 개를 모두 읽어
  두고, 중간에 실패하면 이미 교체한 파일을 원래대로 되돌립니다. 최초
  프로비저닝이라 되돌릴 내용이 없으면 실패한 실행이 만든 파일을 제거합니다.
  되돌리기까지 실패하면 직접 확인해야 할 파일 목록을 오류에 담아 알립니다
