...
반영하고 `bootroot-agent`를 기동합니다. 이후 실행 중인 에이전트는 fast-poll
루프로 스스로 최신 상태를 유지합니다. OpenBao에서 trust와 `secret_id`
회전을 가져와 `agent.toml`을 재렌더하므로 원격 호스트에 두 번째 데몬(OpenBao
Agent 사이드카)이 필요 없습니다. `bootroot-remote apply-secret-id`와
...
- `local-file`인 경우 서비스 머신에서 서비스별 OpenBao Agent 사이드카와
  `bootroot-agent`를 기동/상시 운영하세요. `bootroot service add`가 같은
  호스트에 trust 파일을 미리 준비하므로, 첫 managed `bootroot-agent`
  실행부터 검증 모드로 시작 가능합니다
- `remote-bootstrap`인 경우 생성된 `원격 실행 명령 템플릿`을 수정한 뒤
  서비스 머신에서 `bootroot-remote bootstrap`을 1회 실행하고
  `bootroot-agent`를 계속 실행하세요. 원격 호스트에는 OpenBao Agent
  사이드카가 실행되지 않습니다. 에이전트가 스스로 인증하고 trust와
  `secret_id` 회전을 fast-poll로 가져오므로 수동 재부트스트랩이나
  `apply-secret-id` 없이 회전이 전파됩니다(이들은 `secret_id_ttl`을 넘겨
  오프라인 상태였던 에이전트를 위한 복구 경로일 뿐입니다)
...
등록된 서비스의 **OpenBao Agent**(`bao agent`) 사이드카 컨테이너를
기동하여 rotate 시 시크릿이 즉시 다시 렌더링되도록 합니다.
`secrets/openbao/services/<service>/docker-compose.override.yml`에
서비스별 compose override를 작성하고 `docker compose ... up -d`로
사이드카를 띄웁니다.

> 참고: 이 명령이 관리하는 것은 **OpenBao Agent**이며,
> `bootroot-agent`(인증서 발급/갱신 데몬)와는 다른 바이너리입니다.
> 같은 "agent"라는 단어를 공유하지만 별개의 소프트웨어입니다 —
> 아래 [OpenBao Agent vs `bootroot-agent`](#openbao-agent-vs-bootroot-agent)
> 참고.

### 입력

- `--service-name`: 서비스 이름 식별자(이미 `state.json`에 등록되어
  있어야 합니다)
- `--compose-file`: compose 파일 경로(기본 `docker-compose.yml`)
- `--openbao-network`: 사이드카가 연결될 docker 네트워크(선택)
  - 생략 시 `bootroot-openbao` 컨테이너의
    `com.docker.compose.project` 라벨에서 `<project>_default` 형식으로
    자동 도출합니다.
  - OpenBao를 bootroot compose **외부**에서 운영하는 경우(별도 호스트,
    kubernetes, 매니지드 서비스 등) 필수입니다 — 검사할
    `bootroot-openbao` 컨테이너가 없으므로 네트워크를 명시해야
    합니다.
  - docker 네트워크 이름 규칙(`^[A-Za-z0-9][A-Za-z0-9_.-]*$`)으로
    검증하여 override 파일 인젝션을 방지합니다.

### 동작

다음 결정 표에 따라 docker 네트워크와 compose project를 결정합니다.
첫 두 행의 "OpenBao in compose"는 compose 파일에 `openbao` 서비스가
선언되어 있는지 여부를 의미합니다.
...
| 없음 | 설정 | 전달된 네트워크 사용; `-p` 생략(작업 디렉터리 기반 기본 project로 동작) |

이전에는 compose project를 `bootroot`, 네트워크를 `bootroot_default`로
하드코딩하고 있었습니다. 이번 동작 변경으로 어떤 작업 디렉터리나
...
- 생성되는 사이드카 컨테이너 이름은
  `bootroot-openbao-agent-<service>`입니다.
- `--deploy-type`에 따라 bind-mount가 달라집니다.
  - `daemon`: 서비스별 config/cert 디렉터리를 `/sidecar-config`,
    `/sidecar-certs`로 마운트하여 사이드카가 `agent.toml`,
    `ca-bundle.pem`을 `bootroot-agent`가 읽고 `rotate`가 대기하는
    경로에 직접 렌더링합니다.
  - `docker`: 공용 secrets 디렉터리만 `/openbao/secrets`로
    마운트합니다.
- 성공 시
  `bootroot service openbao-sidecar start: bootroot-openbao-agent-<service> 시작됨`
  메시지를 출력합니다.
...
```bash
# 표준 compose 관리 환경, 작업 디렉터리 무관.
bootroot service openbao-sidecar start --service-name edge-proxy

# 외부 OpenBao(별도 호스트 / kubernetes / 매니지드 서비스).
bootroot service openbao-sidecar start \
  --service-name edge-proxy \
  --openbao-network app-shared-net

# OpenBao가 compose에 있더라도 명시적으로 네트워크를 지정
# (예: 사이드카를 기본 외 네트워크에 연결).
bootroot service openbao-sidecar start \
  --service-name edge-proxy \
...
`service openbao-sidecar start`가 실행하는 것은 **OpenBao Agent**(`bao agent`)
이며, `bootroot-agent`(인증서 발급/갱신 데몬)가 아닙니다. 같은
...
`service openbao-sidecar start`는 첫 번째만 관리합니다.

### OpenBao Agent: 사이드카 vs 호스트 데몬

OpenBao Agent의 두 가지 배포 모델은 서비스의 `--deploy-type`과
독립적입니다. 즉, 서비스가 호스트 데몬으로 동작하더라도 OpenBao
Agent 자체는 bootroot가 관리하는 사이드카 컨테이너로 띄울 수도 있고,
운영자가 직접 관리하는 호스트 데몬(`bao agent
-config=secrets/openbao/services/<svc>/agent.hcl`)으로 띄울 수도
있습니다.
...
| 라이프사이클 관리 | bootroot 소유(`service openbao-sidecar start`) | 운영자 소유(systemd unit 등) |
| 컨테이너 이름 | 표준화: `bootroot-openbao-agent-<svc>` | 운영자가 정의한 PID/유닛 이름 |
| 권한 격리 | 토큰/secret_id가 컨테이너 안에 격리 | 호스트 사용자 권한과 혼합 |
| 코드 경로 일관성 | docker/daemon deploy 타입에 동일 패턴 | daemon deploy 타입에서만 사용 가능 |
| 디버깅 | `docker logs bootroot-openbao-agent-<svc>` | `journalctl -u <unit>`(운영자가 정의) |

사이드카가 **부적합**한 경우:

- docker 엔진이 없는 호스트.
- 토큰을 다루는 프로세스에 대해 systemd 전용 라이프사이클을
  요구하는 보안 정책.
- 사이드카 오버헤드가 허용되지 않는 매우 자원 제한적인 환경.
- 컨테이너 외부에서만 접근 가능한 HSM / 보안 모듈과 통합이
  필요한 경우.
- systemd 기반 시크릿 관리에 표준화된 기존 운영 인프라.

**권장:** 기본은 사이드카입니다. `bootroot service add`도
`bootroot service openbao-sidecar start`를 첫 번째 다음 단계로 안내합니다.
위 조건 중 하나에 해당하는 경우에만 호스트 데몬 대안을 선택하세요.
호스트 데몬을 선택하면 OpenBao Agent 프로세스의 라이프사이클은
