...
프로세스 상시 운영도 운영자 책임입니다: `bootroot-agent`는 호스트 데몬
(예: `Restart=always` 또는 `on-failure`가 설정된 systemd 유닛)으로
실행합니다. 컨테이너로 동작하는 소비 애플리케이션에는 컨테이너 restart
정책과 Docker 데몬 부팅 자동 시작을 함께 적용하세요.

두 전달 모드 모두에서 실행 중인 `bootroot-agent`는 fast-poll 루프로 스스로
최신 상태를 유지합니다. OpenBao에서 trust, `secret_id`, 리스폰더 HMAC,
EAB 회전을 가져와 `agent.toml`을 재렌더하므로 어느 서비스 호스트에도 두
...
bootroot가 만드는 파일은 모두 스테이징 후 이름 변경(stage-then-rename) 방식으로
게시됩니다. 먼저 대상 파일과 같은 디렉터리에 임시 파일로 바이트를 쓰고, 그 임시
이름 상태에서 최종 권한·소유권을 설정하고 flush까지 마친 뒤에야 대상 경로로
rename합니다. 운영 중인 스택 주변에서 알아 두면 좋은 결과는 일곱 가지입니다.

- **읽는 쪽이 잘린 파일을 보지 않습니다.** 파일을 마운트한 컨테이너, 템플릿을
  다시 렌더링하는 사이드카, `.env`를 해석하는 `docker compose`, `agent.toml`을
  다시 읽는 `bootroot-agent` 모두 이전 파일 아니면 완전한 새 파일만 봅니다. 도중에
  실패한 쓰기는 이전 파일을 그대로 두고 임시 파일을 제거합니다.
- **최종 권한이 파일이 나타나는 순간부터 적용됩니다.** 갓 기록된 CA 비밀번호,
  복구 키, OpenBao 언실 키, `secret_id`, 리스폰더 HMAC이 의도보다 넓은 권한으로
  노출되는 구간이 없습니다.
- **rename은 새 inode를 설치합니다.** 매 쓰기마다 대상 경로의 inode가 바뀌므로,
  파일 디스크립터를 열어 둔 쪽(`tail -f`, 기동 시 파일을 연 컨테이너)은 경로를
  다시 열기 전까지 이전 내용을 계속 읽습니다. *디렉터리* 바인드 마운트는 rename을
  따라가지만 단일 *파일* 바인드 마운트는 따라가지 않으므로, 새 버전을 반영하려면
  컨테이너를 재시작해야 합니다.
- **하드 링크는 끊어집니다.** 잘라 쓰기(truncate)는 같은 inode를 가리키는 모든
  하드 링크에 새 내용을 그대로 전달했습니다. rename은 주어진 이름 하나에만 새
  inode를 설치하므로, 나머지 링크는 이전 내용을 그대로 들고 그 쓰기 시점부터
  파일을 따라가지 않습니다. bootroot가 기록하는 경로에서 하드 링크는 지원하지
  않습니다. bootroot가 쓴 파일이 다른 위치에도 있어야 한다면, bootroot가 링크를
  따라 기록하는 경로(아래 참고)에 심볼릭 링크를 두거나, 파일을 만드는 명령을
  실행한 뒤 직접 복사하십시오.
- **ACL, 확장 속성(xattr), SELinux 레이블은 쓰기를 넘어 유지되지 않습니다.**
  이들은 rename이 교체하는 inode에 붙어 있었고, 스테이징한 임시 파일은 아무것도
  붙어 있지 않은 상태로 새로 만들어집니다. 넘어오는 것은 아래에 설명한 규칙에
  따른 권한 비트뿐입니다. `state.json`이나 `.env`에 걸어 둔 POSIX ACL은 다음
  쓰기 이후 아무 안내 없이 사라지고, SELinux 호스트에서는 새 inode가 직접 지정한
  파일 레이블 대신 상위 디렉터리의 기본 레이블을 받습니다. 파일을 다시 쓰는
  명령을 실행할 때마다 다시 적용하거나, 새로 만들어지는 파일이 상속하도록 상위
  디렉터리에 설정해 두십시오.
- **쓰기를 막는 권한이 파일에서 디렉터리로 옮겨 갔습니다.** 잘라 쓰기는
  *파일*에 대한 쓰기 권한이 필요했지만, rename은 파일을 담은 *디렉터리*에 대한
  쓰기 권한이 필요하고 파일 자체에는 아무 권한도 필요하지 않습니다. 그래서 두
  가지 구성에서 동작이 바뀝니다. 읽기 전용 디렉터리 안의 쓰기 가능한 파일은
  전에는 갱신됐지만 이제 임시 파일을 만드는 단계에서 실패합니다. 그리고
  bootroot가 기록하는 경로에 쓰기 잠금 용도로 걸어 둔 `chmod 444`는 이제 아무것도
  막지 못합니다. 쓰기가 파일을 열어서가 아니라 파일을 교체해서 성공하기
  때문입니다. bootroot로부터 경로를 지키려면 상위 디렉터리의 쓰기 권한을
  회수하거나 `chattr +i`로 파일을 불변으로 만드십시오. rename이 여전히 존중하는
  경계는 이 둘입니다.
- **크래시는 잘린 대상 파일이 아니라 `.tmpXXXXXX` 형제 파일을 남깁니다.** 임시
  파일을 만든 뒤 rename하기 전에 장비가 죽으면 대상 경로에는 이전 파일이 그대로
  남고 — 이것이 이 방식의 목적입니다 — 같은 디렉터리에 낯선 `.tmpXXXXXX` 파일이
  함께 남습니다. 이 파일은 쓰기가 도달했던 모드를 그대로 달고 있습니다.
  스테이징 파일이 처음 갖는 `0600`이거나, 완성된 파일의 모드이거나, umask가
  모드를 정하는 파일이라면 umask가 준 값입니다. 이 파일을 읽는 주체도 치우는 주체도
  없으니 직접 삭제하십시오. 실패로 끝난 쓰기는 자신의 임시 파일을 스스로
  지웁니다.

rename 후 상위 디렉터리를 flush할지는 파일마다 따로 정합니다. 그 flush는 매 쓰기
마다 디스크 왕복 한 번을 쓰기 때문입니다.

flush하는 파일 — 전원이 끊겨도 게시된 파일이 남습니다.
...
두 번째 목록은 스스로 다시 만들어집니다. 다음 갱신, OpenBao Agent 사이드카의 다음
렌더링, 또는 해당 명령의 재실행으로 복구됩니다. 첫 번째 목록은 그렇지 않습니다.
bootroot가 재개를 위해 다시 읽는 파일이거나, 스택이 로그인에 사용하는 자격 증명
파일이어서, 하나를 잃으면 다음 쓰기가 아니라 운영자나 다음 회전이 있어야 되돌릴 수
...
출력 두 파일은 `0600`, 발급된 인증서와 CA 번들은 `0644`입니다. 모드를 아무도 정해
두지 않은 파일은 대상에 이미 있는 모드를 그대로 씁니다. 손으로 좁혀 둔 파일은 이후
모든 쓰기에서도 좁은 채로 남고, 새로 만들 때는 프로세스 umask가 허용하는 값 —
일반적인 `open(2)`가 `0666`에 umask를 적용해 내놓는 바로 그 값 — 을 받습니다.
여기에 해당하는 파일은 `state.json`, `ca.json`, `openbao.hcl`, compose 오버라이드,
`init`이 롤백에서 되돌리는 파일이 전부이며, `umask 077`인 호스트에서는 `0600`으로,
기본값인 `umask 022`에서는 `0644`로 만들어집니다.

`.env`는 두 번째 갈래에서 유일한 예외입니다. 이미 있는 파일의 모드를 그대로
유지하는 것은 같지만, 새로 만들 때는 umask에 맡기지 않고 `0600`으로 만듭니다.
`infra install`이 이 파일에 `POSTGRES_PASSWORD`를 기록하는데, 이 파일은 `0700`
시크릿 트리가 아니라 compose 디렉터리에 있기 때문입니다. 이 파일을 읽는 주체 —
`docker compose`와 bootroot 자신 — 는 모두 root로 실행됩니다. 더 넓은 권한이
필요하면 한 번 `chmod` 해 두십시오. 이후 모든 쓰기가 설정한 값을 유지하며, 접근
권한을 잃은 읽기 측은 조용히 넘어가지 않고 다음 compose 실행에서 실패합니다.
같은 이유로 이 동작이 생기기 전에 설치한 환경은 `.env`가 처음 만들어진 모드를
그대로 유지하므로, 이미 디스크에 있는 비밀번호를 좁히려면 직접 `chmod 0600` 을
실행하십시오.

이 경로 중 하나를 심볼릭 링크로 다른 위치의 파일에 연결해 두었다면 동작은 파일에
따라 다릅니다. rename은 링크를 따라가는 대신 주어진 이름 자체를 교체하기
때문입니다.

- **위치를 옮겨 두었을 수 있는 설정 파일은 링크를 따라 기록합니다.** `.env`,
  `ca.json`과 그 템플릿, `openbao.hcl`, HTTP-01 리스폰더 설정과 템플릿, OpenBao
  Agent 설정, 생성된 compose 오버라이드, `state.json`, `init` 출력 두 파일은
  링크를 먼저 해석해 그 대상에 게시하므로 링크는 이전에 가리키던 파일을 그대로
  가리킵니다. 서로를 가리키며 순환하는 링크는 대상이 없으므로 거부됩니다.
  `bootroot-remote bootstrap`이 대상 호스트에 기록하는 `agent.toml`도 여기에
  속합니다. 그 호스트에서 이 파일을 만드는 것이 바로 이 명령이므로,
  `--agent-config-path`에 놓인 링크는 운영자가 직접 마련한 것입니다. 바로 옆의
  `--eab-file-path`는 자격 증명 경로이므로, 그곳의 링크는 대체됩니다.
- **컨트롤 노드 자신의 `agent.toml`, 발급된 인증서와 키, CA 번들, 모든 자격
  증명은 경로 자체에 게시되며**, 그 자리에 있던 링크는 일반 파일로 대체됩니다.
  OpenBao 언실 키와 시크릿 트리의 EAB 파일은 이번 릴리스에서 이 그룹에
  합류했고, 나머지 파일은 이미 여러 릴리스 전부터 rename으로 게시되어 왔습니다.
  이 경로의 링크는 파일을 만드는 명령을 견디지 못합니다. 자격 증명의 경우
  링크를 따라가는 것은 자격 증명 바이트가 bootroot가 관리하는 경로 밖으로
  우회된다는 뜻이기도 합니다. 이 대체는 조용히 일어나지 않습니다. bootroot는
  해당 경로와 링크가 가리키던 파일을 명시한 `WARN`을 한 번 남깁니다. 다만 그
  경고가 실제로 눈에 보이는지는 쓰기를 수행한 프로그램에 따라 다릅니다. 이
  파일들을 기록하는 프로그램 중 로그 구독자를 설치하는 것은 `bootroot-agent`
  뿐이어서, 이 데몬이 서비스 호스트에서 갱신하며 대체한 링크만 다른 로그와 함께
  보고됩니다. `bootroot` CLI와
  `bootroot-remote`는 구독자를 설치하지 않으므로, 이 두 프로그램이 수행하는 대체
  — 즉 실행 중인 데몬이 아니라 운영자 명령이 기록하는 이 그룹의 모든 경로 — 는
  아무 곳에도 표시되지 않은 채 일어납니다. 이 파일들을 기록하는 명령을 실행한
  뒤에 그 자리에 링크를 두었다면 `ls -l`로 경로를 확인하십시오.
...
- `--no-build`: 기본 `--build` 대신 `docker compose up --no-build --pull
  never`를 실행합니다. 이미 로드된 이미지를 그대로 사용하며 태그된
  이미지가 없으면 레지스트리에 접근하지 않고 명확히 실패합니다(누락
  이미지를 조용히 빌드하는 일반 `up`, 또는 Compose 기본 `missing` 풀
  정책에서 누락된 이미지 전용 서비스를 여전히 풀하는 단순 `--no-build`와
...
같은 `init`은 `templates/ca.json.ctmpl`도 유도된 이름 세트로 다시
생성하고 `bootroot-openbao-agent-stepca` 사이드카를 재시작하여 새
템플릿을 읽게 합니다. 이 사이드카는 일정 주기마다 템플릿으로
`ca.json`을 다시 렌더링하므로, 재시작하지 않으면 몇 초 만에 이전 이름
세트가 되돌아오고 step-ca가 다음 재시작에서 주소 SAN을 다시 잃게
됩니다.

같은 `init`의 이후 단계가 실패하면 롤백이 `ca.json`과
`templates/ca.json.ctmpl`을 함께 복원하고 사이드카를 복원된 템플릿으로
다시 재시작하므로, `init` 이전의 이름 세트가 그대로 남습니다. 템플릿과
사이드카는 `ca.json`과 동일한 롤백 범위에 포함되며, `ca.json`보다 한
단계 앞선 상태로 남지 않습니다.
...
- 선언의 schema-1 형식: 필드, 다이제스트, SemVer 버전, 플랫폼.
- `docker-compose.yml`과 `docker-compose.deploy.yml` 양쪽에 걸친 포괄성과
  일관성: 선언된 역할을 실행하는 모든 기본 서비스가 선언된 저장소와 태그를
  가리키고, 선언되지 않은 역할이 실행되지 않으며, 선언된 역할 중 실행되지
  않는 것이 없어야 합니다. 선언된 역할을 실행하는 서비스는 이미지를
  빌드하지 않고 받아 와야 하며, `linux/amd64`가 아닌 플랫폼을 요청해서는
  안 됩니다.
- 환경 격리: 각 Compose 파일을 명시적인 env 파일과 정리된 환경에서
  렌더링하므로, 체크아웃의 `.env`나 호출자 환경의 이미지·프로필 변수가
  소스 기본값을 대신하지 못합니다.
...
발급, CA 키 재생성, step-ca 비밀번호 회전), 그리고 통합 테스트가 실행하는
PostgreSQL·OpenBao 이미지 상수와 `bootroot rotate` 바이너리의 step-ca
비밀번호 헬퍼 호출이 넘기는 이미지가 그 대상입니다. 이 테스트들은 저장소와
태그만 비교합니다.
...
- `--rotate-bound-cidrs`: 두 rotate AppRole 자격증명
  ...
  바인딩할 CIDR 범위 (반복 가능, 예:
  `--rotate-bound-cidrs 10.0.0.5/32`). `state.json`에 기록되어 자체
  발급되는 모든 rotate `secret_id`에 다시 적용됩니다. OpenBao가 컨트롤
  플레인 호스트에 대해 보는 소스 IP를 지정하세요 — 배포 모드에 따라
  달라지므로 bootroot는 자동 유추하지 않습니다. 생략하면 바인딩이
  없습니다(옵트인). 호스트 경계 통제이며 프로세스 격리가 아닙니다.
...
- `--root-token`: KV/AppRole 체크용 토큰
  ...
  상세 체크는 제한됩니다.
- `--agent-config`: 레지스트라 감사 저장소를 찾고 스캔하는 데 쓸 설정을 선택하는
  선택적 `bootroot-agent` 구성 파일입니다. 생략하면 `status`는 `agent.toml`이
  있으면 이를 사용하고, 없으면 문서화된 기본 저장소 위치를 스캔합니다.

### 출력

- 컨테이너 상태 요약
- OpenBao/ KV 상태 요약
- 마지막 AppRole `secret_id` 회전 성공 시각(`state.json`에 기록된
  경우), 그리고 그 타임스탬프가 rotate 역할 `secret_id` TTL의 절반보다
  오래되면(기본 `24h` → `12h` 초과 시) 데드맨 **경고** — 예약된 회전
  작업이 조용히 멈췄을 수 있습니다.
  [운영 > 데드맨 모니터링과 비상 복구](operations.md#데드맨-모니터링과-비상-복구)를
  참고하세요.
- 레지스트라 감사 저장소 상태: 짝 없는 intent, 잘못된 레코드 수, 보존 부족
  여부를 보고합니다. 이 섹션은 항상 표시하며, 없는 저장소는 구성되지 않음으로,
  스캔 실패는 경고로 표시합니다.
...
- 컨테이너 미기동/비정상
- OpenBao 응답 불가
- 지정한 `--agent-config` 경로가 없는 경우

데드맨 경고는 명령을 실패로 만들지 않습니다. 조치가 필요한 정보성
출력입니다.
...
새로운 서비스가 step-ca에서 인증서를 발급받을 수 있도록
...
- 자동 반영: `agent.toml`의 관리 대상 프로필 블록 갱신(없으면 추가),
  ...
  가져온 `[acme].http_responder_hmac`, 에이전트의 fast-poll 자체 인증
  루프를 활성화하는 `[openbao]` 섹션(`url`, `kv_mount`, `role_id_path`,
  `secret_id_path`, `ca_bundle_path`, 그리고 `agent.toml` 옆의 서비스별
  절대 경로 `state_path`, 예:
  `/etc/bootroot/bootroot-agent-state-<service>.json`), 서비스 `secret_id`
  옆의 `eab.json` 프로비저닝(KV에 EAB가 없으면 제거),
  `bootroot-http01` 컨테이너에 HTTP-01 DNS 별칭 자동 등록
...
- `local-file`인 경우 `service add`가 출력한 실행 명령
  (`bootroot-agent --config <agent.toml> --eab-file <eab.json>` — 아래
  참고)으로 `bootroot-agent`를 호스트 데몬으로 기동/상시 운영하세요.
  `bootroot service add`가 같은 호스트에 trust 파일과 `[openbao]`
  fast-poll 섹션을 미리 준비하므로, 첫 managed `bootroot-agent`
  실행부터 검증 모드로 시작 가능합니다
- `remote-bootstrap`인 경우 생성된 `원격 실행 명령 템플릿`을 수정한 뒤
  서비스 머신에서 `bootroot-remote bootstrap`을 1회 실행하고
  `bootroot-agent`를 계속 실행하세요
...
EAB 갱신과 `rotate eab-clear`가 해당 에이전트에서 조용히 무시됩니다.
...
- `local-file` 방식: trust 설정(`trusted_ca_sha256`, `ca_bundle_path`)이
  ...
  `ca_bundle_path`에 기록되며, 실행 중인 에이전트의 fast-poll 루프가
  이를 계속 동기화합니다.
...
`bootroot-agent`는 **호스트 데몬**(예: systemd 유닛)으로 실행됩니다.
Docker 사이드카로 실행하지 않으며, 서비스별 OpenBao Agent도 실행되지
않습니다: 두 전달 모드 모두에서 데몬 자신의 fast-poll 루프가 시크릿을
전달합니다. 한 호스트는 **서로 다른 서비스마다** `bootroot-agent`
프로세스 하나와 agent 구성 하나를 실행합니다 — `[openbao]` 섹션은
AppRole 자격증명을 하나만 담으므로 서로 다른 서비스가 하나의
`agent.toml`을 공유할 수 없습니다. 하나의 구성에 여러 `[[profiles]]`를
두는 것은 같은 서비스의 인스턴스에 대해서만 지원됩니다
([운영 > systemd 운영 절차](operations.md) 참고).

컨테이너로 동작하는 소비 애플리케이션은 계속 지원됩니다: 호스트 데몬이
앱 컨테이너가 bind-mount하는 호스트 디렉터리에 인증서를 기록하고,
post-renew 훅이 컨테이너를 리로드합니다
...
프리셋(`--reload-style`/`--reload-target`)과 저수준
커스텀 명령(`--post-renew-*`)을 한 번의 호출에서 함께
지정하여 **두 개**의 갱신 후 훅을 등록할 수 있습니다 —
예를 들어 하나의 갱신에서 컨테이너 소비자를 재시작하고
*동시에* 다른 소비자를 리로드할 수 있습니다. 둘 다
지정하면 방출 순서는 규칙으로 고정됩니다(clap이 복구할 수
없는 CLI 위치가 아니라): **프리셋 항목이 먼저, 그다음
커스텀 명령 항목**이 기록됩니다. 프리셋만 지정하면
동등한 단일 저수준 훅으로 확장됩니다. 이 플래그는
`remote-bootstrap` 전달 모드에서 `bootroot-remote
bootstrap`으로도 전달되며, 그곳에서 모든 훅이 순서대로
보존됩니다.

*커스텀* 명령을 둘 이상(또는 프리셋을 둘 이상) 한 번의
호출에서 등록하는 것은 지원되지 않습니다. 위의 "프리셋
1개 + 커스텀 1개" 조합을 제외하면 플래그는 단일 값으로
유지됩니다.
...
원자성: 키 파일, 인증서, CA 번들 모두 stage-then-rename 방식으로
기록됩니다 — `state.json`과 `init` 출력 파일을 기록하는 것과 동일한
게시 루틴을 통과합니다. 같은 디렉터리의 임시 파일을 `O_CREAT|O_EXCL`,
`mode=0600`으로 먼저 생성하고, (정책이 활성화된
경우) 그 임시 파일에 `chown`을 적용한 뒤 최종 모드(정책이 활성화된
키는 `0640`, 그렇지 않으면 `0600`, 인증서와 번들은 `0644`)로
설정하고, 마지막으로 목적지에 `rename`합니다. 두 가지가 따라옵니다.
목적지 경로는 최종 정책보다 넓은 모드로 노출되는 순간이 존재하지
않습니다 — 클램프 전 umask 기반의 `0644` 윈도우도, chown 전 운영자
기본 gid 하에서 group-readable로 잠시 노출되는 윈도우도 없습니다.
그리고 회전 중에 목적지를 읽는 소비자 — 리로드되는 서버나 신뢰
저장소를 다시 구성하는 agent — 는 이전 파일이나 완전한 새 파일 중
하나만 보게 되며, 잘린 PEM을 보는 일은 없습니다.

이 rename들 이후에는 상위 디렉터리를 의도적으로 flush하지 않습니다.
rename을 잃는 크래시는 이전 인증서/키/번들을 그대로 남기고 다음
갱신이 재발급하므로, 장애가 아니라 재발급 한 번의 비용에 그칩니다.
반면 bootroot가 다시 읽어 이어서 진행하는 `state.json`과 `init`
출력 파일은 그 flush를 수행합니다.
...
- 다른 서비스가 이미 사용 중인 `--agent-config` 경로로 `local-file`
...
- 전달 모드 요약(`local-file`은 자동 반영된 `agent.toml` 경로와 자동
  프로비저닝된 `eab.json` 경로, `remote-bootstrap`은
  부트스트랩 아티팩트 + 순서형 원격 handoff 명령 템플릿 출력)
- 출력에 소유/책임 범위를 명시하는 라벨을 함께 표시:
  `Bootroot 자동 반영 항목`, `운영자 실행 항목 (필수)`,
  `운영자 실행 항목 (권장)`, `운영자 실행 항목 (선택)`
- 복붙용 스니펫: daemon 프로필 블록, trust 설정, daemon 실행 명령
...
- `--reload-style`: 갱신 후 훅의 리로드 스타일 프리셋
  ...
  저수준 `--post-renew-*` 플래그와 함께 지정하여 프리셋 훅과
  커스텀 명령 훅을 한 번의 업데이트에서 등록할 수 있습니다.
  재작성되는 훅 목록은 `service add`와 동일하게 프리셋이 먼저,
  그다음 커스텀 항목 순서로 방출됩니다.
...
훅 플래그가 하나라도 지정되면 `service update`는 해당 서비스의
전체 훅 목록을 재작성합니다(기존의 "재작성 vs 유지" 규칙).
모든 훅 플래그를 생략하면 현재 훅은 변경되지 않습니다.
...
서비스를 등록 해제하고 해당 `OpenBao` 자료를 정리합니다. 이는 등록된
서비스의 `--delivery-mode`를 변경하는(예: `local-file` →
`remote-bootstrap`) 지원되는 방법입니다. 서비스를 제거한 뒤 새 모드로 다시
추가하세요. `service add`는 기존 항목의 delivery-mode 전환을 거부하고(중복
서비스 오류로 종료) 따라서 제거 후 재추가가 의도된 흐름입니다. delivery-mode
변경은 어차피 `secret_id` 전달 자료를 재생성하므로, 재추가로 생성되는 새
`AppRole` `role_id`는 정상입니다.
...
- `state.json`에서 서비스 항목을 읽어 제거 계획을 출력합니다: `AppRole`과
  정책(상태에 저장된 이름 기준), 서비스별 KV 경로, 그리고
  `--delete-artifacts` 사용 시 삭제할 디스크 파일.
- 원격 정리를 먼저 수행합니다: 서비스별 KV 경로(`eab`,
  `http_responder_hmac`, 트러스트 항목, 그리고 `remote-bootstrap`
  서비스의 경우 `secret_id`), `AppRole`, 정책을 삭제합니다. 각 삭제는
  이미 없는 리소스를 허용하므로 부분 실패 후에도 안전하게 재실행할 수
  있습니다.
- `state.json` 항목은 **마지막에** 제거합니다. 모든 원격(그리고
  `--delete-artifacts` 시 디스크) 정리가 성공한 뒤에만 제거됩니다. 부분
  실패는 항목과 저장된 role/policy 이름을 남겨두어 재실행이 남은 삭제를
  마칠 수 있게 합니다.
- `bootroot-http01` 응답기의 HTTP-01 별칭 집합을 갱신하여 제거된 서비스의
  별칭을 제거합니다. **결과 별칭 집합이 비게 되는 경우에도** 마찬가지입니다
  (마지막 별칭 보유 서비스를 제거해도 응답기는 기본 별칭만으로 다시
  연결됩니다).
...
- 제거 계획
- 리소스별 제거됨 / 이미 없음 라인
- 성공 확인, 또는 부분 실패 시 남은 리소스 보고와 재실행 안내
...
- `state.json` 누락
- 등록되지 않은 서비스
- 원격 또는 디스크 삭제 실패(재실행을 위해 `state.json` 항목은 유지됨)
...
- daemon 프로필/실행 명령 스니펫(실행 명령에는 `--eab-file`이 포함되며,
  EAB 회전이 적용되려면 필수입니다)
...
- `--auth-mode`: 런타임 인증 모드 (`auto`, `root`, `approle`, 기본값 `auto`)
- `--root-token`: OpenBao root token (CLI 플래그, 전환/비상 경로).
  `--root-token-file`과 함께 쓸 수 없습니다.
- `--root-token-file`: OpenBao root token이 들어 있는 파일 경로.
  우선순위는 `--root-token-file` > `--root-token` >
  `OPENBAO_ROOT_TOKEN` 환경 변수 > 대화형 프롬프트입니다.
  파일은 다른 사용자 읽기 권한이 없어야 합니다 (`0o644`는 거부되며
  `chmod 0600` 안내가 출력됩니다). 그룹 공유 CI/운영 계정을 위한
  `0o640`은 허용됩니다.
- `--approle-role-id`: OpenBao AppRole role_id
  (환경 변수: `OPENBAO_APPROLE_ROLE_ID`)
- `--approle-secret-id`: OpenBao AppRole secret_id
...
- `--yes` / `-y`: 확인 프롬프트 생략. `rotate` 하위에서 위치에 상관없이
  허용됩니다 (예: `rotate force-reissue --yes`나
...
서비스 일괄, 또는 장기 실행 인프라 OpenBao Agent
...
- `--infra <stepca|responder>`: 대상 인프라 역할
  ...
  플래그로 전달해 인증합니다.
- `--rotate-bound-cidrs`: `bootroot-infra-rotate-role` 자격증명을
  바인딩할 CIDR 범위 (반복 가능). `--infra`와 함께만 유효하며 루트 토큰
  프로비저닝 실행에서만 적용됩니다. 바인딩은 `state.json`에 기록되어
  발급되는 운영자 자격증명과 이후의 모든 자체 재발급에 적용됩니다.
  생략하면 기록된 바인딩이 유지됩니다(재적용된 바인딩이 출력됩니다).
- `--clear-rotate-bound-cidrs`: 기록된 `--rotate-bound-cidrs` 바인딩을
  제거하고 운영자 자격증명을 바인딩 없이 발급합니다. `--infra`와
  함께만 유효하며 루트 토큰 프로비저닝 실행에서만 적용되고
...
KV는 전혀 읽을 수 없습니다. 각 자격증명은 **자기 자신의** `secret_id`만
재발급할 수 있습니다 — 예약된 회전 작업이 스스로 만료되지 않게 하는
자체 재발급입니다.

자체 재발급(mint-own-last): 실행이 rotate AppRole로 인증했고 해당
호출의 모든 대상이 성공하면, 명령은 인증에 사용한 자격증명을
재발급하고(`num_uses = 6`, 기록된 `--rotate-bound-cidrs` 바인딩 재적용)
새 `secret_id`를 로그인으로 검증한 뒤 `--approle-secret-id-file`로
전달된 파일을 원자적으로 교체합니다. 인라인/환경 변수로 공급된
secret_id에는 교체할 파일이 없으므로 경고를 출력하고 자체 재발급을
건너뜁니다. 루트 토큰 실행은 자체 재발급을 하지 않습니다. 성공한 호출은
`state.json`에 `last_secret_id_rotation`도 기록하며, `bootroot status`가
이를 데드맨 신호로 감시합니다.
[운영 > rotate 자격증명 자체의 secret_id (자체 재발급)](operations.md#rotate-자격증명-자체의-secret_id-자체-재발급)을
참고하세요.
...
`0600`) 기록하고, `role_id` 파일이 없으면 백필하며, 인프라 에이전트가
...
회전을 범위가 제한된 자격증명으로 전환할 수 있습니다. 프로비저닝
실행은 `init --secret-id-ttl`로 기록된 `secret_id` TTL을 유지하며
역할을 기본값으로 되돌리지 않으므로, 실제 자격증명의 수명이 `bootroot
status`가 `state.json`에서 유도하는 임계값과 계속 일치합니다. AppRole
...
`local-file` 서비스의 경우 기록된 cert/key 파일을 삭제하고
bootroot-agent 호스트 데몬에 SIGHUP(데몬 설정 경로에 대한
`pkill -HUP`)을 보내 다음 루프 틱에서 재발급을 수행합니다.
...
- `--requester`: 재발급 KV 페이로드에 기록할 운영자 라벨(선택). 지정하지
  ...
  사용됩니다.
- `--wait`: bootroot-agent가 완료를 보고할 때까지(또는 타임아웃까지)
  대기합니다. `--delivery-mode remote-bootstrap` 서비스는 KV 재발급
  경로의 `completed_at`을 폴링하고, local-file 서비스는 `paths.cert`
  파일의 시리얼이 바뀔 때까지 폴링합니다(드물게 동일 시리얼로
  재발급되는 경우만 mtime을 보조 지표로 사용). 성공 시 요약 줄에는
  종단 간 지연(remote는 KV 페이로드의 `completed_at - requested_at`,
  local은 신호 직후부터 새 인증서가 기록될 때까지의 wall-clock 시간)이
  사람이 읽기 쉬운 형식으로 함께 출력됩니다.
- `--wait-timeout`: `--wait`가 설정된 경우 최대 대기 시간. humantime
  형식(예: `90s`, `2m`)을 사용합니다. 기본값 `2m`.

`--wait` 없이 실행하면 KV 쓰기(remote) 또는 SIGHUP/재시작 신호(local)
직후 즉시 반환합니다. bootroot-agent는 다음 polling 주기 안에 재발급을
반영합니다. `--wait` 타임아웃은 오류가 아닙니다: 요청은 큐에 남아
있으므로 agent는 다음 polling에서 여전히 처리합니다.
...
- Phase 6 — trust 확정: 최종 trust(신규 fingerprint만)를 OpenBao에 기록해
  ...
  확정된 `agent.toml` trust 핀과 `ca-bundle.pem`을 반영합니다
...
알려진 모든 EAB KV 경로의 자격증명을 비워서 에이전트가 더 이상
오래되었거나 잘못된 EAB 자료를 사용하지 않도록 합니다. 제거된
`rotate eab`의 후속이며, `--no-eab` 및 EAB 입력 검증으로 닫은 증상의
복구 경로입니다(#588 §3c).
...
- 전파는 fast-poll의 몫입니다: 각 `bootroot-agent`(로컬 호스트 데몬과
  ...
  필요 없습니다.
- 비워진 값은 `--eab-file`과 함께 기동된 에이전트에만 도달합니다
...
- 항목별 재발급 디스패치 — 기록 위치는 디스패치되는 함수에 의해
  ...
  - `openbao` → OpenBao 서버 인증서를 새로 발급해
    ...
    없고 소유자가 이미 올바르면 아무 일도 하지 않습니다.
    `<compose-dir>/openbao/tls` 자체가 심볼릭 링크이면 아무것도
    마운트하기 전에 발급을 거부합니다(`Refusing to use a symlink as
    the OpenBao TLS output directory`). 마운트 원본 경로는 최종
    구성요소까지 해석되므로, 이 위치의 링크는 chown과 인증서 기록을
    모두 링크 대상으로 옮기게 됩니다. bootroot는 이 경로를 항상 실제
    디렉터리로만 생성하며, 상위 경로의 심볼릭 링크는 기존과 동일하게
    해석됩니다. 두 파일은
...
- AppRole secret_id 회전 시 AppRole 로그인 점검 결과(`--infra` 대상은
  일치하는 인프라 OpenBao Agent 재시작 포함)
...
- `--root-token-output <path>` *(옵트인)*: 새로 발급된 루트 토큰을 제한된
  ...
  토큰 파일은 stage-then-rename 방식으로 기록됩니다: 목적지와 같은
  디렉터리에 `0600`으로 생성된 임시 파일에 토큰을 쓰고 flush한 뒤
  목적지로 `rename`하며, rename 이후 상위 디렉터리도 flush합니다.
  따라서 새로 발급된 루트 토큰이 프로세스 umask 기본 권한으로
  노출되는 순간이 없고, 목적지 이름이 일부만 기록된 토큰을 가리키는
  순간도 없으며, 공개된 파일은 전원 손실에도 살아남습니다 — reinit이
  다시 발급해 주지 않는 자격증명의 유일한 사본이기 때문입니다. 기존
  대상이 있으면 그 안에 남아 있는 이전 토큰을 위해 먼저 `0600`으로
  좁힙니다. 대상이 일반 파일을 가리키는 심볼릭 링크인 경우도 계속
  지원됩니다: 링크를 해석해 그 대상 파일에 토큰을 기록하므로, rename은
  링크가 아니라 사전 검사가 판정한 파일을 교체합니다. 링크가 서로를
  가리키며 순환하는 경우에는 기록할 대상 파일 자체가 없으므로 사전
  검사가 해당 경로를 거부합니다 — 이전의 truncate 방식이 wipe 이후에
  `ELOOP`으로 실패하던 지점을, wipe 이전으로 옮긴 것입니다. init 이후 쓰기가
  그래도
...
- `--summary-json <path>`: `init`으로 그대로 전달되며, 파괴적 동작
  ...
  `chmod 0600` 안내와 함께 거부됨)을 가지거나, 상위 디렉터리가 쓰기
  불가능/생성 불가능한 경우 reinit이 시작되지 않습니다. summary JSON
  에는 새로 발급된 루트 토큰과 unseal key가 포함되므로, 쓰기 불가능한
  대상은 partial-init 트랩을 다른 출력 채널을 통해 재현하게 되고,
  `0600`보다 넓은 권한을 가진 대상은 이전 실행이 남긴 summary — 그
  자체로 자격증명을 담고 있는 파일 — 를 호스트의 모든 사용자에게
  읽히도록 방치한 상태입니다. summary 파일 자체는 루트 토큰 파일과
  동일한 방식으로 기록됩니다: 목적지와 같은 디렉터리의 `0600` 임시
  파일에 기록하고 flush한 뒤 목적지로 `rename`하며, rename 이후 상위
  디렉터리도 flush합니다. 따라서 JSON이 더 넓은 권한으로 디스크에
  머무는 순간도, 목적지 이름이 일부만 기록된 summary를 가리키는
...
- `socket_path`는 설치된 소켓 유닛의 실효 `ListenStream=` 값이며, systemd가
  해석하는 방식 그대로 해석합니다. 유닛 디렉터리는 systemd 자체
...
- `--socket-unit`이 읽을 수 없는 파일이거나 `[Socket] ListenStream=`이 없는
  유닛을 가리키는 경우
- 설치된 유닛이나 그 드롭인 중 하나를 읽을 수 없는 경우
- 드롭인까지 병합한 뒤에도 설치된 유닛이 아무 경로도 바인딩하지 않는 경우
...
`--json` 없이 실행하면 같은 값 세 개를 담은 현지화된 문장 하나를 출력합니다.
표면 버전 `bootroot.registrar.v1`(와이어 표기 그대로), 조합된 식별자, 그리고
RFC 3339 형식의 리프 만료 시각입니다. 본문을 요청하지 않은 운영자도 어떤 표면
버전이 자격 증명을 발급했는지 알 수 있습니다.

### 동작

- 발급을 시작하기 전에 출력 대상 세 개가 서로 다른 파일인지 확인합니다.
...
- 게시는 되돌릴 수 있습니다. 첫 파일을 교체하기 전에 대상 세 개의 내용과 권한
  비트와 소유권을 모두 읽어 두고, 중간에 실패하면 이미 교체한 파일을 원래
  내용으로, 원래 권한 비트로, 원래 uid와 gid 소유로 되돌립니다. setuid·setgid·
  sticky 비트는 `chown`이 지우므로 소유권을 되돌린 뒤에 다시 적용합니다. 손으로
  좁혀 둔 인증서가 이 명령이 쓰는 `0644`로 돌아오거나, 다른 계정이 소유하던 키가
  실패한 실행의 소유로 돌아오지 않습니다. 최초 프로비저닝이라 되돌릴 내용이
  없으면 실패한 실행이 만든 파일을 제거합니다. 권한이 없어 `chown`이 거부된
  경우를 포함해 되돌리기까지 실패하면, 직접 확인해야 할 파일 목록을 오류에 담아
  알립니다
- 같은 경로에 게시하는 작업은 데몬의 게시를 포함해 서로 직렬화됩니다. 각 실행은
  기록 대상 디렉터리마다 `.bootroot-registrar-publish.lock` 파일에 배타적
  `flock`을 걸고, 대상을 읽어 두기 전부터 마지막 기록이 끝날 때까지 유지합니다.
  따라서 뒤에 온 쪽은 기다렸다가 앞선 쪽의 결과 위에 세 파일을 통째로 게시하며,
  앞선 쪽의 중간에 끼어들지 않습니다. 데몬도 시작 시 이 쌍을 발급할 때와 갱신할
  때 같은 대상에 같은 잠금을 걸므로, 데몬이 실행 중인 호스트에서 이 명령을
  실행해도 데몬의 인증서와 키 사이에 끼어들지 않고 그 반대도 마찬가지입니다.
  잠금 파일은 내용이 없고 `0600`으로 만들어지며 이후 지우지 않습니다. 잠금은
  프로세스가 끝나면 커널이 해제합니다. 게시에 앞선 발급은 직렬화하지 않습니다.
  발급은 프로세스마다 이름이 다른 스테이징 디렉터리에만 기록하기 때문입니다
- 재실행하면 같은 경로에 재발급하며, 키는 매번 새로 생성됩니다

### 실패 조건

- `--host`가 단일 DNS 레이블이 아니거나 `--domain`이 유효한 DNS 이름이 아닌 경우
- 출력 대상 세 개 중 둘이 같은 파일인 경우
- 출력 대상 이름이 `.bootroot-registrar-publish.lock`인 경우. 이 명령이 기록
  대상 디렉터리마다 거는 게시 잠금 파일의 이름입니다
...
- `--ca-bundle-path`: 관리되는 step-ca trust bundle을 쓸 출력 경로.
  `--artifact` 미지정 시 필수.
- 갱신 후 훅 플래그: `--post-renew-command`,
  `--post-renew-arg`, `--post-renew-timeout-secs`,
  `--post-renew-on-failure`. 이 개별 필드 플래그는
  **단일 훅**이며 `--artifact`가 없을 때만 사용됩니다.
  `--artifact`가 제공되면 그 `post_renew_hooks` 배열이
  기준이 되며, 배열이 담고 있는 **모든** 훅이 순서대로
  원격 `agent.toml`에 렌더링됩니다 — 따라서 프리셋과
  커스텀 명령으로 등록된 서비스(`bootroot service add`
  참조)는 원격 호스트에서 두 훅을 모두 유지합니다. (이
  비대칭성은 `service add` CLI와 동일합니다: 아티팩트
  경로는 다중 훅, 개별 필드 플래그는 그렇지 않습니다.)
