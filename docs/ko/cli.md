...
- **컨트롤 노드 자신의 `agent.toml`, 발급된 인증서와 키, CA 번들, 모든 자격
  ...
  경고가 실제로 보이는 곳은 구성 요소에 따라 다릅니다. 로그 구독자를 설치하는
  것은 장시간 실행되는 구성 요소뿐이어서, `bootroot-agent`와 HTTP-01 리스폰더는
  이를 다른 로그와 함께 출력하지만 `bootroot` CLI는 아직 구독자를 설치하지 않아 자신이
  기록하는 경로 — 언실 키 파일, 시크릿 트리의 EAB 파일, 컨트롤 노드의
  `agent.toml` — 에 대해서는 아무것도 보여 주지 않습니다. 이 파일들을 기록하는
  CLI 명령을 실행한 뒤에 그 자리에 링크를 두었다면 `ls -l`로 경로를 확인하십시오.
