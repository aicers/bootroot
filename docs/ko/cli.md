...
bootroot service agent start ...`) 없이 동작합니다.
...
- `--deploy-type`에 따라 bind-mount가 달라집니다.
  ...
  - `docker`: 공용 secrets 디렉터리만 `/openbao/secrets`로
    마운트합니다.
- 성공 시
  `bootroot service agent start: bootroot-openbao-agent-<service> 시작됨`
  메시지를 출력합니다.
...
`service agent start`가 실행하는 것은 **OpenBao Agent**(`bao agent`)
...
`service agent start`는 첫 번째만 관리합니다.
...
`bootroot service agent start`를 첫 번째 다음 단계로 안내합니다.
