...
설치, 구성, 운영하는 전체 과정을 설명합니다. PKI 배경 지식이 없는 운영자도
...
운영 대상 애플리케이션(daemon/docker 배포 대상)을 뜻합니다. Bootroot의 역할은
...
때문입니다. 그래서 Bootroot의 일부 과정은 운영자가 수동으로 할 수밖에
...
SecretID의 회전과 전달은 운영자가 직접 Bootroot CLI(`bootroot` &
...
OpenBao에 접속하여 시크릿을 받아 오는 것은 인프라 OpenBao Agent와 각
서비스 `bootroot-agent`의 fast-poll 루프가 담당합니다.

### Bootroot가 자동화하지 않는 것 (1) — OpenBao의 Unseal Keys와 SecretID

시크릿 매니저 그 자체를 사용하기 위한 시크릿은 어쩔 수 없이 운영자가
수동으로 관리해야 합니다.
...
**언실 키.** OpenBao를 실행시킬 때에만 필요하므로 운영자가 직접 따로 잘
...
**SecretID.** AppRole과 SecretID는 EAB와 마찬가지로 OpenBao에 대한 인증
수단입니다. 보안상 중요한 대목은 SecretID의 회전입니다. Bootroot는 SecretID
회전을 자동화해 주지 않습니다. 왜냐하면 SecretID가
다른 머신으로 전달되어야 하는 경우에는 이를 안전하게 하기 위해 추가적인
관리 체계를 도입해야 하기 때문입니다.

#### 언실 키와 루트 토큰의 회전

언실 키와 루트 토큰의 교체는 운영자가 명시적으로 수행하는 수동 작업이며,
자동 스케줄로 진행되지 않습니다.

루트 토큰은 언실 키 없이 교체할 수 있지만, 언실 키는 현재 유효한 키 조각을
임계치 이상 확보해야만 교체할 수 있고 이를 분실했다면 OpenBao 재초기화 후
Bootroot 초기화와 서비스 재bootstrap을 다시 진행해야 합니다.

### Bootroot가 자동화하지 않는 것 (2) — CA 키 회전

CA 키 회전은 자동 스케줄로 진행되지 않으며 운영자가 계획된 작업 시간대에
수동으로 수행해야 합니다. 이 작업은 인증서 신뢰 체인 전환과 서비스 영향 점검이
동반되므로, 운영자가 사전 점검과 실행 후 검증을 포함한 운영 절차를 직접
책임져야 합니다.

### Bootroot가 자동화하지 않는 것 (3) — 설치와 운영 관련

설치와 운영 측면에서 Bootroot의 자동화 범위가 아닌 것이 있으므로 유의해야
합니다.

- **설치**: Bootroot CLI와 `bootroot-agent`의 설치는 운영자가 직접
  해야 합니다. (CA를 포함하는 인프라
  구성 요소들과 이들을 위한 OpenBao Agent는 최초 설치 시
  `bootroot infra install`로, 이미 구성된 환경의 재시작 시
  `bootroot infra up`으로 자동으로 설치/기동됩니다.)
- **프로세스 관리**: 운영자가 직접 설치한 `bootroot-agent` 데몬이 계속
  실행될 수 있도록 systemd 또는 도커 restart 설정도
  운영자가 직접 설정 또는 확인해야 합니다.
- **회전 스케줄링**: Bootroot는 회전 명령(`bootroot rotate`)을 제공하지만
  그 주기적 실행(cron, systemd timer 등)은 운영자가 설정해야 합니다.
...
- 운영자가 책임지는 것: 실행 구성요소 설치/업데이트(bootroot 관련 실행
  파일), 프로세스 상시 실행 보장, 실행 환경 구성(예: Compose
...
동작할 수도 있고, 다른 머신에서 동작할 수도 있습니다. 두
`--delivery-mode` 모두 런타임 구성은 동일합니다. 서비스는
bootroot-agent만 실행하며, 이 에이전트가 AppRole로 OpenBao에 직접
인증하고 fast-poll 루프로 시크릿을 최신으로 유지합니다(서비스별 OpenBao
Agent 없음).

bootroot-agent 배치 규칙:

- **bootroot-agent는 호스트 데몬**(systemd)으로 실행합니다 — Docker
  사이드카로는 실행하지 않습니다. 서로 다른 서비스마다 `bootroot-agent`
  프로세스 하나와 agent 구성 하나를 사용하세요. `[openbao]` 섹션은
  AppRole 자격증명을 하나만 담으므로 서로 다른 서비스가 하나의
  `agent.toml`을 공유할 수 없습니다(여러 `[[profiles]]`는 같은 서비스의
  인스턴스에 대해서만 지원됩니다).
- 인증서를 소비하는 애플리케이션은 컨테이너로 실행해도 됩니다. 호스트
  데몬이 컨테이너가 bind-mount하는 호스트 디렉터리에 인증서를 기록하고,
  `docker-restart` post-renew 훅이 컨테이너를 재기동합니다.
...
1. CA 인프라 구동 및 초기화
2. 서비스 추가 및 서비스별 bootroot-agent 설정
3. 인프라 OpenBao Agent가 렌더링한 파일로 step-ca/responder에 시크릿을
   주입하고, 각 서비스의 bootroot-agent는 fast-poll 루프로 OpenBao에서
   자신의 시크릿을 직접 가져옴
