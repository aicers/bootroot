...
인증서 발급과 회전을 해 주는 시스템이다. 보안 측면에서 강조되는 대목은
회전(rotation)이고, 갱신(renewal)이 만료에 대응하는 것이라면 회전은 만료
여부와 관계없이 주기적으로 교체한다는 점이 다르다. 회전을 구현하는 방법은
두 가지가 있다. 하나는 만료 여부와 관계없이 일정 주기마다 교체하는 것이고,
다른 하나는 유효 기간을 짧게 설정하여 만료 전에 갱신함으로써 결과적으로
주기적 교체가 이루어지게 하는 것이다. Bootroot는 후자의 방식을 사용한다.

### Bootroot의 2차 목표 — 자동화

초기 발급과 주기 회전의 전체 과정을 최대한 안전하게 자동으로 해 주는 것
역시 Bootroot의 중요한 목표이다. 자동화가 안 되면 불편할 뿐만 아니라
사람이 수동으로 하면서 보안 허점을 만들기 쉽다. CA로부터 인증서를 새롭게
받아오는 역할을 `bootroot-agent` 데몬이 자동으로 수행한다.

### 100% 자동화는 보안상 불가능

그러나 완전한 자동화는 보안 관점에서 보면 불가능하다. 최초 신뢰 획득과
같은 절차마저 자동화를 함부로 하다 보면 그 자체가 보안 취약점이 될 수 있기
때문이다. 그래서 Bootroot의 일부 과정은 사용자가 수동으로 할 수밖에
없는데, Bootroot는 이 수동 작업을 최대한 줄이도록 설계했다.

### Bootroot가 자동화하지 않는 것 (1) — CA의 키 갱신

현재 Bootroot는 CA의 비밀키/공개키가 최초 생성된 이후에 이를 변경하는 것을
별도의 기능으로 제공하고 있지 않다. CA의 키를 변경하려면 초기화 과정부터
다시 진행해야 한다.

### 인증서 회전 자동화를 달성하기 위한 파생 목표 — 시크릿 관리

Bootroot가 사용하는 CA 프로토콜 ACME 방식은 CA가 요청자로부터 EAB
kid/hmac을 받아서 인증한 뒤에 인증서를 발급 또는 갱신해 준다. EAB
kid/hmac은 일종의 아이디와 비밀번호 같은 것으로서 `bootroot-agent`가 CA에
이 값을 제대로 전달해야 하고, 이 자동화 과정에서 이를 안전하게 관리해야
한다. 따라서, 시크릿 매니저가 필요하고 Bootroot는 OpenBao를 사용한다.
(안전하게 관리해야 할 비밀번호 같은 것을 시크릿이라 부른다.)

### Bootroot의 3차 목표 — 시크릿 회전

시크릿 역시 인증서와 마찬가지로 보안 측면에서 강조되는 대목은 회전이다.
시크릿 매니저는 안전하게 보관해 주는 역할을 할 뿐이므로, 자동 회전은
Bootroot가 수행한다.
...
OpenBao를 사용하려면 언실 키와 루트 토큰이 필요하다. 언실 키는 OpenBao를
처음 실행시킬 때 필요한 것이고, 루트 토큰은 OpenBao와 상호 작용을 할
때마다 필요하다.

**언실 키.** OpenBao를 실행시킬 때에만 필요하므로 사용자가 직접 따로 잘
기억해 두고 수동으로 입력해야 한다. (개발 편의를 위해서 언실 키를 파일에
쓰고 OpenBao가 구동될 때 그것을 읽어서 입력하는 기능이 제공된다.)

**루트 토큰.** Bootroot는 초기화 과정에서만 루트 토큰을 사용하고, 이후
일상 운영에서는 루트 토큰이 필요하지 않다. 대신 AppRole과 SecretID 입력
방식을 기본으로 사용한다. AppRole과 SecretID는 EAB와 마찬가지로 OpenBao에
대한 인증 수단이다. 보안상 중요한 대목은 SecretID의 회전이다. Bootroot는
SecretID 회전을 자동화해 주지 않는다. 왜냐하면 SecretID가 다른 머신으로
전달되어야 하는 경우에는 이를 안전하게 하기 위해 추가적인 관리 체계를
도입해야 하기 때문이다.

### 시크릿의 자동 회전과 수동 회전

SecretID의 회전과 전달은 사용자가 직접 Bootroot CLI(`bootroot` &
`bootroot-remote`) 명령을 사용해서 수행해야 한다. SecretID를 제외한 다른
시크릿들은 모두 Bootroot가 자동으로 회전 및 전달을 수행한다. SecretID는
OpenBao 자체에 대한 접근을 위한 것이지만, 나머지 다른 시크릿들은
OpenBao에 접속하여 받아올 수 있으므로 회전 과정의 자동화가 가능한 것이다.
OpenBao에 접속하여 시크릿을 받아 오는 것은 OpenBao Agent가 담당한다.

### Bootroot가 자동화하지 않는 것 (3) — 최초 CA 인증

시크릿 매니저 측면만이 아니라 인증서 발급/갱신 측면에서도 최초 신뢰
문제는 있다. `bootroot-agent`가 CA와 통신할 때 올바른 CA인지 확인을 해야
하는데, 이를 위해서는 CA의 인증서를 미리 배포해야 한다. 그런데, 이 과정을
수동으로 하는 것은 불편하고, 자동으로 하는 경우에는 이 배포 과정을 안전하게
수행하는 별도의 체계가 필요하게 된다. 다행히도 Bootroot의 기본 환경은
서비스를 처음 설치할 때 CA와 `bootroot-agent`를 사용자가 동시에 직접
제어하는 환경이니 최초 인증서 발급 시 CA 확인을 생략해도 별 문제가 없다.
이 최초 과정에서 CA 인증서를 받도록 되어 있으니 이후 자동화된 서비스 인증서
회전 과정에서 CA 확인이 담보된다.
...
- **설치**: Bootroot CLI와 각 서비스를 위한 OpenBao Agent 및
  `bootroot-agent`의 설치는 사용자가 직접 해야 한다. (CA를 포함하는 인프라
...
- **프로세스 관리**: 사용자가 직접 설치한 OpenBao Agent와
  ...
  사용자가 직접 설정 또는 확인해야 한다.
- **회전 스케줄링**: Bootroot는 회전 명령(`bootroot rotate`)을 제공하지만
  그 주기적 실행(cron, systemd timer 등)은 사용자가 설정해야 한다.
