...
`bootroot rotate ca-key`는 중간 CA 키 쌍을 교체하고,
`bootroot rotate ca-key --full`은 루트와 중간 CA 키 쌍을 모두
교체합니다. 두 모드 모두 8단계 절차(백업, 생성, 가산 신뢰, step-ca
재시작, 재발급, 신뢰 확정, 정리)를 사용하며, 각 단계는 멱등성을
보장하므로 실패 후 같은 명령을 다시 실행하면 중단된 지점부터 자동으로
재개됩니다. `rotation-state.json` 파일이 진행 상태를 기록하고 동시
수정을 방지합니다. 전체 모드(full)는 4-핑거프린트 전환 신뢰를 사용하여
마이그레이션 기간 동안 클라이언트가 인증서를 검증할 수 있게 합니다.
...
### Bootroot가 자동화하지 않는 것 (2) — 최초 CA 인증
