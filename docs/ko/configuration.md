...
것입니다. 키는 정확히 세 개이며, 알 수 없는 키는 설정 오류입니다.

**프로덕션 요청 핸들러가 추가되기 전까지 활성화는 계속 지원되지
않습니다.** 이 빌드에는 버전이 지정된 레지스트라 페이로드 코덱이 포함되어
있지만 동사를 호출하는 핸들러는 등록되어 있지 않습니다. 따라서 활성화된
...
**서버 자재는 대역 외로 공급합니다.** 이 리프에 대한 인증서 발급이
들어오기 전까지 bootroot는 이 파일들을 만들지 않습니다. 직접 배치하거나
테스트에서는 픽스처가 만들어 둡니다. 데몬은 자체 서명 인증서를 만들지
않고, 호스트의 일반 서비스 리프로 대체하지 않으며, 스스로 발급하지도
않습니다.

엔드포인트가 활성화된 상태에서 다음은 각각 **기동 거부**이며, 진단
메시지는 문제가 된 설정 키를 항상 밝히고 그 설정에 값이 있으면 설정된
경로도 함께 밝힙니다. `server_cert_path` 또는 `server_key_path`가 없음,
설정된 경로의 자재가 없음·읽을 수 없음·파싱 불가·키 불일치·SAN 불일치,
위 자가 점검이 거부하는 자재, 그리고 `trust.ca_bundle_path`가
없음·읽을 수 없음·파싱 불가·핀에 맞는 인증서 없음.
...
**두 인증서 경로도 같은 이유로 고정됩니다.** 둘 중 하나라도 바꾸는
