...
- **`client_cert_path`** 는 레지스트라 자신의 **클라이언트** 리프 PEM이며,
  그 유일한 DNS SAN은
  `<instance>.bootroot-registrar.<host>.<network.domain>`입니다. 이는 내부
  구현 세부가 아니라 공개된 계약입니다. 같은 호스트의 레지스트라 프로세스가
  엔드포인트에 인증하기 위해 이 파일을 읽고, 프로비저닝 도구가 설치 시점에
  *최초* 인증서를 이 경로에 배치합니다. 또한 엔드포인트의 앵커 핀 파일
  `registrar-endpoint-anchors.sha256`을 찾는 위치가 이 인증서와 같은
  디렉터리이므로, 인증서를 옮기면 핀 파일도 함께 옮겨집니다.
...
네 자재 경로는 **서로 다른 네 개의 파일**을 가리켜야 하며, 그중 둘이 같은
파일을 가리키는 설정은 설정 검증 시점에 거부됩니다. 진단 메시지는 충돌한
설정 키들과 이들이 공유하는 파일을 함께 밝힙니다. 서버 쌍과 클라이언트
쌍은 서로 독립적으로 분류되고 차례로 발급되므로, 경로를 공유하면 마지막에
쓰인 것만 남습니다. 그리고 그 아래 어느 단계도 이를 문제 삼지 않습니다.
각 파일은 개별적으로는 흠 없는 상태이기 때문입니다. 엔드포인트는 로드되고
키와도 맞는 리프를 제시하며 기동하고, 레지스트라는 자기 것이 아닌 신원으로
인증하게 됩니다. 비교는 `.`·`..`을 정규화한 뒤의 문자열 비교이며 파일을
열지는 않습니다. 이 파일들을 발급하는 기동 시점에는 아직 존재하지 않아도
되기 때문입니다.
...
#### 두 리프는 데몬이 스스로 발급합니다 {#the-daemon-issues-both-leaves-itself}

두 이름 모두 예약된 `bootroot-` 네임스페이스에 속하므로
`bootroot service add`로는 어느 쪽도 만들 수 없습니다. 즉 이 이름들을
발급할 수 있는 주체는 bootroot 자신의 데몬뿐입니다. 기동 시,
엔드포인트의 TLS 자재를 로드하기 **전에**, `bootroot-agent`는 사용할 수
없는 리프를 발급해 각 리프와 키를 설정된 고유한 경로 쌍에 씁니다. 이때
쓰는 자격 증명은 데몬 자신의 bootroot-internal 특권 자격 증명 —
`auth/cert`에서 `OpenBao`에 인증하는 root 소유 신원 — 이며, 경로는 다른
모든 발급과 동일한 로컬 step-ca로의 아웃바운드 ACME 경로입니다. 이
경로에는 `AppRole`도, 만료되는 비밀도 없습니다. 데몬이 자체 서명 인증서를
만들지 않고 호스트의 일반 서비스 리프로 대체하지도 않는다는 점은 그대로
입니다.

데몬이 쓰는 인증서 파일은 **리프 다음에 CA가 돌려준 발급자 체인**을
담으며, 그 발급자는 `trust.ca_bundle_path`에도 병합됩니다. 이
핸드셰이크의 양쪽 모두 읽는 쪽 디스크의 번들이 아니라 자재가 *제시하는*
체인에서 신뢰 앵커를 고르므로, 리프만 담긴 파일은 문제없이 로드된 뒤
거부됩니다. 위 `server_cert_path` 설명이 요구하는 바로 그 조건이며,
이제는 파일을 놓아 둔 주체가 아니라 데몬이 충족합니다. 리프만 필요한
읽는 쪽은 첫 번째 PEM 블록에서 그대로 찾을 수 있습니다.

쓸 체인이 아예 없는 경우가 하나 있습니다. CA가 **발급자를 하나도 붙이지
않고** 리프만 돌려준 경우입니다. 이때 발급은 경고를 남기고
`trust.ca_bundle_path`를 건드리지 않은 채 리프만 게시하며, 방금 쓴 서버
파일에 대해 엔드포인트의 TLS 로더가 같은 체인 요구 사항으로 기동을
거부합니다. 이는 사용할 수 없는 여덟 가지 상태가 아니라 CA 쪽의 이상이라
다시 발급한다고 복구되지 않습니다. 기동 진단이 해당 파일을 밝히며,
바뀌어야 하는 것은 CA가 내려주는 체인입니다.

엔드포인트가 **비활성화된** 호스트는 아무것도 발급하지 않고, 자재 경로를
만들지 않으며, CA에도 `OpenBao`에도 아무것도 요청하지 않습니다.

**사용할 수 있는 자재는 그대로 둡니다.** 설정된 경로의 한 쌍은 다음이 이
순서대로 모두 성립할 때 사용할 수 있습니다. 두 파일이 존재하고, 두 파일을
읽을 수 있으며, 두 파일이 파싱되고, 키가 리프의 키이며, 리프의 DNS SAN이
정확히 하나이고 그것이 해당 쌍의 예약된 이름이며, 리프의 `not_before`가
지났고, `not_after`가 아직 지나지 않았으며, `trust.ca_bundle_path`가
설정되어 있다면 그 앵커까지 체인이 이어집니다. 사용할 수 있는 쌍은
재발급하지 않으므로, 재시작이 레지스트라가 읽고 있는 파일을 흔들지도,
그 프로세스에 새 키를 떠안기지도 않습니다.

**그 밖의 상태는 불평 대신 교체합니다.** 각각은 평범한 사고가 남기는
상태이고, 거부하면 데몬이 실행되어야만 고칠 수 있는 상태에 호스트를
가둬버리기 때문입니다. 조건마다 하나씩 여덟 가지가 있습니다. **없음**,
**읽을 수 없음**, **파싱 불가**, **키 불일치**, **SAN 불일치**,
**아직 유효하지 않음**, **만료됨**, **체인 이탈** — 데몬은 이 여덟 가지
모두에 대해 발급을 시도합니다. *파싱 불가*는 쓰기 도중 전원이 끊긴 갱신이
남기는 상태이고, *키 불일치*는 인증서 rename과 키 rename 사이에서 죽은
갱신이 남기는 상태이며, *만료됨*은 `not_after`를 지나도록 내려가 있던
데몬이 돌아왔을 때의 상태이고, *체인 이탈*은 번들이 더 이상 갖고 있지 않은
CA 세대가 서명한, 시간상으로는 유효하고 이름도 맞는 리프입니다.

다음 세 경우는 데몬 기동으로 이어지지 않으며, 서로 구분해 두는 편이
좋습니다.

- **쓰기가 이뤄질 수 없는 경우** — 경로가 디렉터리이거나, 끊어진
  심볼릭 링크이거나, 불변 파일이거나, 데몬에게 없는 권한이 필요한 경우.
  이는 발급 실패이며, 진단 메시지가 경로와 쓰기 오류를 밝힙니다.
- **`trust.ca_bundle_path`를 읽을 수 없는 경우.** 발급이 필요하다고
  판단된 뒤 **게시 이전에** 실패합니다. 데몬은 내용을 확인할 수 없는
  번들을 덮어쓰기를 거부하므로, 리프는 하나도 쓰이지 않고, 번들을 밝히는
  진단과 함께 기동이 거부되며, 번들은 바이트 단위로 그대로 남습니다.
  *없는* 번들과 *읽을 수는 있으나 파싱되지 않는* 번들은 다른 경우이며
  데몬 기동으로 이어집니다. 아래를 보십시오.
- **교체본이 여전히 유효 기간 밖인 경우** — 이 호스트의 시계 기준으로요.
  발급은 성공했고, 그 뒤 엔드포인트의 TLS 로더가 서버 리프를 거부한
  것입니다. 이는 발급이 새로 추가한 거부가 아니라 로더가 원래 갖고 있던
  거부입니다. 호스트와 CA 사이의 지속적인 시계 편차는 양방향 모두 여기에
  도달합니다. 시계가 충분히 뒤처지면 교체본이 아직 유효하지 않게 되고,
  충분히 앞서면 이미 만료된 상태가 됩니다.

**앵커 파일이 없거나 파싱되지 않으면 거부가 아니라 복구합니다.** 같은
`trust.ca_bundle_path`가 두 가지 역할을 겸합니다. 리프를 대조하는 앵커
집합이자, 아웃바운드 ACME 연결과 HTTP-01 responder 등록이 자신의 TLS를
앵커하는 파일입니다. 그래서 리프를 발급해야 하는데 그 파일이 없거나,
있어도 파싱되는 인증서를 하나도 담고 있지 않으면, 데몬은 발급에 앞서
bootroot-internal 자격 증명 자신의 CA 번들에서 그 파일을 복구합니다. 이때
`trust.trusted_ca_sha256`이 이미 핀으로 지정한 앵커만 남기며, 그렇게 했다는
사실을 로그로 남깁니다. 이어지는 발급은 여느 발급과 똑같이 CA가 돌려준
체인을 복구된 파일에 병합합니다. 파싱되는 번들은 앵커가 아무리 낡았더라도
이 복구가 건드리지 않으며 — 그것을 옮기는 일은 병합의 몫입니다 — 읽을 수
없는 번들도 마찬가지로 위의 거부에 맡깁니다.

**ACME 입력은 이 파일이 아니라 `OpenBao`에서 옵니다.** 계정을 등록할 때
쓰는 EAB와 HTTP-01 responder 등록 요청에 서명하는 HMAC은 배포
`state.json`이 기록한 KV 마운트 아래의 `bootroot/agent/eab`와
`bootroot/responder/hmac`에서, bootroot-internal 자격 증명을 통해
읽습니다. 대체 경로는 없습니다. 이 파일의 `[acme] http_responder_hmac`도
`[eab]` 블록도 대신 쓰이지 않으며, 읽은 값을 디스크에 되쓰지도 않습니다.
이 읽기는 리프를 실제로 발급해야 할 때에**만** 일어나므로, 자재가 멀쩡한
호스트는 `OpenBao`에 닿지 못해도 기동합니다. 읽기 실패 — `OpenBao`에 닿을
수 없음, `auth/cert` 로그인 거부, KV 레코드 없음, 페이로드 파싱 불가 — 는
실패한 읽기 **그리고** 그로 인해 발급되지 못한 자재 경로를 함께 밝히는
진단과 함께 기동을 거부합니다.

EAB를 등록하지 않은 배포는 레코드가 없는 배포가 아닙니다. `bootroot
init`은 모든 배포에서 `bootroot/agent/eab`를 씁니다. `--no-eab`, reinit,
프롬프트 거절로 등록할 것이 없었던 경우에는 비워진 형태 — `kid`와 `hmac`이
모두 빈 값이며, `bootroot rotate eab-clear`가 쓰는 것과 같은 페이로드 —
를 담습니다. 그 비워진 형태는 "이 배포에는 EAB가 없다"로 읽습니다. 그런
배포에서 이미 다른 모든 발급이 그러하듯 외부 계정 바인딩 없이 계정을
등록하고, 데몬은 그렇게 하고 있다는 사실을 로그로 남깁니다. 따라서 레코드가
**없는** 경우는 그 답이 아니라 실패한 읽기이며 — 마운트가 지워졌거나,
경로가 삭제되었거나, `kv_mount`가 잘못된 경우가 남기는 상태입니다 — 바인딩
없는 계정으로 두 표면 리프를 조용히 발급하는 대신 거부합니다. 레코드가 있는데
파싱되지 않는 경우도, `kid`만 있거나 `hmac`만 있는 경우를 포함해 거부합니다.

이 경로의 거부 중 하나는 버그가 아니라 의도된 것입니다. 전체 신뢰 회전의
중반부터 마무리 사이에는 내부 자격 증명이 저장하고 있는 루트 지문이 아직
*이전* 루트를 가리키며, 그 구간에서 발급된 리프는 `auth/cert` 항목이 아직
신뢰하지 않는 루트에 체인이 이어집니다. 그 구간에 발급이 필요한
엔드포인트 활성 기동은 자격 증명 자신의 복구 진단과 함께 거부됩니다.
회전을 끝내는 것이 해결책입니다.

##### 인증서 준비가 추가하는 두 가지 거부 {#the-two-refusals-certificate-preparation-adds}

인증서 준비가 엔드포인트 활성 기동을 거부하는 방법은 정확히 두 가지이며,
순서가 있습니다.

1. **필수 `[registrar_endpoint]` 경로가 설정되지 않음** — 무엇을 읽거나
   요청하기도 전에 설정 검증 시점에 결정되며, 빠진 키를 진단 메시지가
   밝힙니다. 네 경로 중 어느 것에도 기본값은 없고, 설정되지 않은 경로는
   복구 가능한 상태가 아닙니다. 발급이 쓸 곳이 없기 때문입니다.
2. **발급 자체가 실패함** — CA에 닿을 수 없음, `OpenBao` 읽기 실패, 로그인
   거부, 상태 파일이나 내부 설정을 읽을 수 없음, 대체된 자격 증명, 또는
   쓰기가 이뤄질 수 없음 — 자재 경로와 실패를 밝히는 진단과 함께.

**이 둘은 인증서 준비가 추가하는 거부일 뿐, 활성화된 엔드포인트가 기동에
실패하는 유일한 두 가지가 아닙니다.** 그렇게 읽으면 다른 이유로 실패한
운영자를 엉뚱한 곳으로 보내게 됩니다. 기존 거부는 모두 그대로입니다.
위 둘보다 앞서는 **비-Linux 플랫폼** 검사, `[registrar] state_file`과
`[registrar]`·`[trust]` 테이블 자체 규칙을 포함한 나머지 **설정 검증**,
디스크립터·주소·소켓 경로의 소유자와 권한에 대한 **소켓 활성화**,
위 사용 가능성 규칙보다 엄격한 수용 규칙을 가진 **엔드포인트의 TLS
로더** — 그래서 발급이 사용 가능하다고 판단한 쌍도 몇 줄 뒤에 거부될 수
있습니다 — 그리고 핸들러가 만들어질 때 뒤늦게 거부하는 **레지스트라
표면의 나머지**(상태 파일, 시크릿 디렉터리, 프로비저닝 설정과 그 다이제스트
게이트, 감사 저장소, 동사 제한기)가 여기에 해당합니다.

##### 이미 만료된 리프는 기동 시점에 복구합니다 {#an-already-expired-leaf-is-repaired-at-start}

`bootroot-agent`가 기동할 때 이미 만료되어 있는 리프는 데몬의 첫 갱신
틱이 아니라 **그 기동 시점에**, 엔드포인트가 TLS 자재를 로드하기 전에
복구됩니다. 첫 틱까지 미루면 활성화된 엔드포인트가 갱신 리드 타임 내내
서비스를 하지 못한 채 남습니다. 그것도 운영자가 복구하려고 수행한 바로 그
재시작에서 말입니다. 그 사이 배포 전체의 등록이 실패하는 동안 정작 고칠 수
있는 기계는 놀고 있게 됩니다.

이로써 **바뀌지 않는** 것은 누가 복구하는가입니다. 여전히
`bootroot-agent`이며, 호스트 재프로비저닝은 결코 아닙니다. 움직인 것은
데몬 자신의 수명 주기 안에서 *언제* 복구가 일어나는가뿐입니다.

데몬이 엔드포인트를 띄우지 못하면 데몬 자체가 올라오지 않으므로, 호출자는
계속 엔드포인트가 도달 불가하다고 보고하며, 해결책은 기동 진단이 밝히는
내용입니다. 이는 발급 실패보다 넓은 범위를 덮습니다. 발급은 성공했지만 그
교체본을 TLS 로더가 거부한 경우 — 호스트 시계가 CA와 어느 방향으로든
충분히 어긋난 경우 — 도 같은 곳에 도달하며 밖에서는 구분되지 않습니다.
CA 장애를 가정하지 말고 진단을 읽으십시오.
...
`client_cert_path`·`client_key_path` 중 하나라도 없음, 그 넷 중 둘이 같은
파일을 가리킴, 위 자가 점검이 거부하는 자재, `trust.ca_bundle_path`가
없음·읽을 수 없음·파싱 불가·핀에 맞는 인증서 없음, 그리고 발급 실패.

설정된 경로에 *있는* 자재가 없음·읽을 수 없음·파싱 불가·키 불일치·SAN
불일치인 경우는 이 목록에 **없습니다**. 위에서 설명한 대로 이제는 각각
발급으로 복구됩니다.
...
앞에서 한 번만 발급하고 읽으므로, 리로드가 바꾼 경로는 아무도 읽지
않습니다. 그 경로의 *내용*은 고정이 아닙니다. 인증서 갱신은 재시작 없이
제시되는 자재를 교체합니다. 고정되는 것은 경로뿐입니다.
