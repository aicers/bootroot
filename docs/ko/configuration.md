...
- **읽을 수 없는 `trust.ca_bundle_path`** 는 발급이 필요하다고 올바르게
  ...
  남습니다.
- **없거나 읽을 수는 있지만 파싱되지 않는 `trust.ca_bundle_path`** 는
  registrar 표면의 기동 시 발급에서 복구할 수 있습니다. Bootroot는 이
  발급(그리고 HTTPS HTTP-01 응답자)에 대해 미리 설정된
  `trust.trusted_ca_sha256` 핀만으로 제시된 step-ca 체인을 인증한 뒤,
  반환된 핀 고정 체인을 검증하고 병합한 다음에만 리프와 키를 게시합니다.
  시스템 루트를 쓰거나 피어에게서 신뢰를 배우지 않습니다. 핀 집합이 비어
  있거나 맞지 않으면 안전하게 실패하며, 읽을 수 없는 번들은 위에서 설명한
  별도의 실패 상태로 남습니다.
...
- `audit_store_enforcement` (기본값 `filesystem`) —
  ...
  재시작해야 합니다. `directory` 모드와 비활성 엔드포인트는 검사하지 않습니다.
...
- `rate_limit_coalesce_window_seconds` (`u64`, 기본값 `60`) — 억제된 호출을
  하나의 감사 레코드로 묶는 도착 기준 창입니다. 양의 정수여야 합니다.

모두 부호 없는 정수이며, 속도는 분수 비율이 아니라 토큰당 밀리초 간격으로
표현해 설정 표면에 부동소수점 값이 하나도 없도록 했습니다. 다섯 키 모두
0은 로드 시점에 거부됩니다. 버스트가 0이면 첫 정상 호출부터 제한되고,
간격이 0이면 토큰이 무한히 공급되어 속도 제한기가 조용히 무력화되며,
coalescing 창이 0이면 제한된 호출을 묶을 수 없기 때문입니다. 음수나 소수
값도 마찬가지로 문제의 키 이름과 함께 거부됩니다.
