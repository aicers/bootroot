...
데몬이 쓰는 인증서 파일은 **리프 다음에 발급자 체인**을 담으며, 발급자는
`trust.ca_bundle_path`에도 병합됩니다. 이 핸드셰이크의 양쪽 모두 읽는
쪽 디스크의 번들이 아니라 자재가 *제시하는* 체인에서 신뢰 앵커를
고르므로, 리프만 담긴 파일은 문제없이 로드된 뒤 거부됩니다. 위
`server_cert_path` 설명이 요구하는 바로 그 조건이며, 이제는 파일을 놓아
둔 주체가 아니라 데몬이 충족합니다. 리프만 필요한 읽는 쪽은 첫 번째 PEM
블록에서 그대로 찾을 수 있습니다.
