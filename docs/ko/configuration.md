...
- **`trust.ca_bundle_path`를 쓸 수 없는 경우.** *읽을 수 없는* 번들은
  발급이 필요하다고 판단된 뒤 **게시 이전에** 실패합니다. 데몬은 내용을
  확인할 수 없는 번들을 덮어쓰기를 거부하므로, 리프는 하나도 쓰이지 않고
  번들도 그대로 남습니다. *없는* 번들과 *파싱되지 않는* 번들도 같은 방식으로
  판단되고 병합 자체는 둘 다 복구하겠지만, 아웃바운드 ACME 클라이언트가
  자신의 TLS를 같은 파일에 앵커하므로 흐름이 시작되기 전에 먼저 거부합니다.
  따라서 세 경우 모두 번들을 밝히는 진단과 함께 기동이 거부됩니다. 이는
  엔드포인트가 활성화된 호스트라면 어차피 도달하는 결과입니다. 엔드포인트의
  TLS 로더도 읽을 수 있고 파싱되며 핀에 맞는 번들을 요구하기 때문입니다.
...
수 없음, `auth/cert` 로그인 거부, `bootroot/responder/hmac` 레코드 없음,
페이로드 파싱 불가 — 는 실패한 읽기를 밝히는 진단과 함께 기동을 거부합니다.

`bootroot/agent/eab` 레코드가 **없는** 경우만은 실패가 아니라 답입니다.
`bootroot init --no-eab`, reinit, EAB 프롬프트 거절이 모두 이 경로를 쓰지
않은 채로 남기며, 그런 배포에서 이미 다른 모든 발급이 그러하듯 외부 계정
바인딩 없이 계정을 등록합니다. 데몬은 그렇게 하고 있다는 사실을 로그로
남깁니다. 레코드가 있는데 파싱되지 않으면 — `kid`만 있거나 `hmac`만 있는
경우를 포함해 — 여전히 거부하며, 명시적으로 비워진 형태(`bootroot rotate
eab-clear`가 쓰는, `kid`와 `hmac`이 모두 빈 값)는 손상이 아니라 "이 배포에는
EAB가 없다"로 읽습니다.
