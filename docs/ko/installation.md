...
프로젝트 기본 `docker-compose.yml`에는 compose 스택을 상대로 **1회
발급**(`--oneshot`)을 수행하는 데모용 `bootroot-agent` 컨테이너가
포함되어 있습니다(`agent.toml.compose`를 읽습니다):

```bash
docker compose up --build -d bootroot-agent
```

이 컨테이너는 데모/스모크 용도로만 존재합니다. 온보딩된 서비스의 실행
모델이 **아니며**, 운영 서비스는 위에서 설명한 bootroot-agent 호스트
데몬으로 실행합니다.
