...
서비스 단위 에이전트는 `bootroot service openbao-sidecar start
--service-name <service>` 사용을 권장합니다. 이 명령은 per-service compose override를
작성하고 bootroot 인프라와 동일한 docker 네트워크에 사이드카를
연결합니다. 네트워크 이름과 compose project는 런타임에
`bootroot-openbao` 컨테이너의 `com.docker.compose.project` 라벨에서
자동으로 도출되므로, 작업 디렉터리나 `COMPOSE_PROJECT_NAME` 값과
무관하게 동작합니다. 입력값과 실패 조건은
...
사이드카를 사용할 수 없는 환경을 위한 참고용입니다. `<network>`
...
- 서비스 OpenBao Agent(권장): `--deploy-type`과 무관하게 서비스별
  ...
  `bootroot service openbao-sidecar start`를 다음 단계로 안내합니다. OpenBao
