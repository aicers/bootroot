...
In bootroot, certificates are issued per service, so DNS SANs include
`service_name`. The same service can also run with multiple instances on a
single machine, so `instance_id` is prefixed to distinguish instances. The
same format is used for both daemon and docker deployments.
...
Because Bootroot does not use an external CA, the root CA is step-ca's
...
CA. It contains the public key and requested
