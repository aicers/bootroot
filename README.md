...
- `./scripts/check-docs.sh`: what CI runs — verifies the vendored theme, builds
  with `--strict`, and asserts the theme stylesheets reached `site/`.
- `./docs/theme/build-docs-pdf.sh en|ko`: builds PDF manuals into `site/pdf/`.
  Needs `pip install mkdocs-with-pdf`.

The shared docs theme is vendored under `docs/theme/` and committed, so a fresh
clone builds the manual with no network access and no `gh`. `mkdocs.yml`
inherits `docs/theme/mkdocs-base.yml` for the theme, markdown extensions, and
stylesheets; keep only site-specific keys in `mkdocs.yml`.

To move to a new theme release, edit `version` in `docs/theme.toml`, run
`./scripts/fetch-theme.sh` (this needs `gh`), and commit the resulting
`docs/theme/`. Never edit files under `docs/theme/` by hand — the installer
records a digest in `docs/theme/.meta` and CI fails on any drift. Changes
belong in [aicers/docs-theme](https://github.com/aicers/docs-theme).
