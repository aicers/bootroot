...
//! `.d/` drop-ins from every unit directory are merged on top — the
//! unit's own, the dash-truncated `bootroot-.socket.d/`, and the
//! type-wide `socket.d/` every socket unit on the host shares. With no
...
/// The suffix every drop-in directory carries — the unit's own
/// `bootroot-registrar.socket.d`, where `systemctl edit
/// bootroot-registrar.socket` writes its override, and the less
/// specific directories [`dropin_dirs`] derives beside it.
...
/// pathname this host does not bind. On top of it go every drop-in
/// systemd would apply to it, from every unit directory — the unit's
/// own `.d/`, the dash-truncated prefixes and the type-wide
/// `socket.d/`, in the order [`dropin_paths`] resolves them. That is
/// how an override written by `systemctl edit` reaches the answer:
/// without it a deployment that moved the socket in a drop-in would be
/// told the packaged path while systemd bound another, and the caller
/// would connect to a socket that is not there.
///
...
fn resolve_socket_path(explicit: Option<&Path>, unit_dirs: &[PathBuf]) -> Result<String> {
    ...
    for dropin in dropin_paths(unit_dirs)? {
        // A drop-in that is not a regular file is systemd's masking
        // form — a symlink to `/dev/null`. It has already taken its
        // name out of the merge; it carries no setting to read.
        if !dropin.is_file() {
            continue;
        }
...
/// Returns the drop-in directory names that apply to the socket unit,
/// in systemd's own order — most specific first.
///
/// systemd looks for more than the directory named after the unit. For
/// a name carrying dashes it also reads the directories generated by
/// repeatedly truncating that name after each dash, so
/// `bootroot-registrar.socket` is extended by `bootroot-.socket.d/`
/// too, and an equally named drop-in further down the hierarchy
/// overrides one further up.
///
/// Templates are not expanded: the unit this reports for is not an
/// instance, and no `@` name reaches here.
fn unit_dropin_names(unit: &str) -> Vec<String> {
    let mut names = vec![unit.to_string()];
    let Some((prefix, unit_type)) = unit.rsplit_once('.') else {
        return names;
    };
    let mut prefix = prefix.to_string();
    // systemd chops a trailing dash once and retries rather than
    // emitting a name for it, so `foo--.service` yields `foo-.service`
    // and not `foo-.service` twice.
    let mut chopped = false;
    while let Some(dash) = prefix.rfind('-') {
        if dash == 0 {
            break;
        }
        if dash + 1 < prefix.len() || chopped {
            prefix.truncate(dash + 1);
            names.push(format!("{prefix}.{unit_type}"));
            chopped = false;
        } else {
...
/// Returns the drop-in directories that apply to the socket unit, in
/// systemd's own precedence order.
///
/// The order is systemd's: every unit directory in turn contributes the
/// unit's own `.d` and then its dash-truncated prefixes, and the
/// type-wide `socket.d/` directories come last across all of them,
/// because that one is the most generic and is meant to be overridden
/// by anything named after the unit.
fn dropin_dirs(unit_dirs: &[PathBuf]) -> Vec<PathBuf> {
    let names = unit_dropin_names(SOCKET_UNIT_FILE);
    let mut dirs: Vec<PathBuf> = Vec::new();
    for dir in unit_dirs {
        for name in &names {
            dirs.push(dir.join(format!("{name}{DROPIN_DIR_SUFFIX}")));
        }
    }
    if let Some((_, unit_type)) = SOCKET_UNIT_FILE.rsplit_once('.') {
        for dir in unit_dirs {
            dirs.push(dir.join(format!("{unit_type}{DROPIN_DIR_SUFFIX}")));
        }
    }
    dirs
}
...
///
/// systemd collects `*.conf` from every directory [`dropin_dirs`]
/// names, keeps the highest-precedence directory's copy of a given
/// filename and discards the rest, then applies what is left sorted by
/// that filename. Both halves matter: the dedup is what lets an
/// operator neutralise a packaged drop-in by putting an empty file of
/// the same name under `/etc`, and the sort is what makes `10-` land
/// before `20-` wherever each came from — including from the type-wide
/// `socket.d/`, whose files are ordered among the rest by name and only
/// lose to a name-specific directory carrying the same filename.
///
/// The type-wide directory is read because systemd reads it: an
/// operator who resets and rebinds `ListenStream=` there has moved this
/// host's socket, and reporting the unit-local path would name a socket
/// nothing is listening on. A name claimed by a drop-in that is not a
/// regular file — the `/dev/null` symlink systemd documents for masking
/// one — still masks the copies below it and contributes no setting of
/// its own, which is how a type-wide drop-in is cancelled for this unit
/// alone.
///
/// # Errors
///
...
fn dropin_paths(unit_dirs: &[PathBuf]) -> Result<Vec<PathBuf>> {
    ...
    for dropin_dir in dropin_dirs(unit_dirs) {
        ...
        for entry in entries {
            ...
            // Deliberately not filtered by file type. A drop-in
            // symlinked to `/dev/null` masks the copies below it by
            // occupying the name, not by dropping out of the list, so
            // it has to reach the dedup below like any other.
