...
/// The registrar's correlation handle for one endpoint response.
///
/// A verb mints it at entry, while the refusing endpoint handler mints it
/// when no verb runs. It correlates a response with its audit record where
/// there is one and with the daemon log line for an unaudited refusal where
/// there is not. It is not a secret or an idempotency key, and nothing is
/// cached under it.
