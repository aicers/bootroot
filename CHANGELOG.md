...
- `bootroot registrar` provisions the registrar surface a deployment tool
  ...
  pathname is read from the installed socket unit, in systemd's own
  unit-directory precedence order, falling back to the unit bootroot ships.
  ...
  and the endpoint's own recognition rule cannot disagree. The material is
  staged and published only once all of it exists, so a run that fails
  part-way leaves no half-written pair. Only the initial credential is issued;
